...
def build_prepare(lang):
    ...
    if not tpl:
        return None
    # ЯЗЫК СТРАНИЦЫ ПОДГОТОВКИ — АНГЛИЙСКИЙ НА ВСЕХ ЯЗЫКАХ САЙТА. Решение владельца 25.08:
    # «может быть пока на английском, статьи принимаем тоже на английском — не хочется
    # держать такой механизм на многих языках, так же как кабинет совета: пока не
    # заработало; заработает — потом дополним». Поэтому строки берутся из en напрямую,
    # а не через _strings(lang): иначе на русской странице половина текста была бы
    # английской, а половина русской — хуже обоих вариантов.
    # (До 25.08 страница вообще печатала сырые «$prep_h1»: шаблон написан 7 августа,
    # а ключи prep_* не завели ни разу, и на проде это висело на пяти языках.)
    s = {**_strings("en"), **{k: v for k, v in _strings(lang).items()
