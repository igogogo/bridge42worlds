...
def cmd_daily(args):
    ...
    # -1 = периметр не дал НИ ОДНОГО кандидата. Возвращаем это наружу отдельным кодом,
    # а не общей неудачей. Планировщик пишет код в logs/daily-history.log, и «rc=0»
    # трое суток подряд означало «всё хорошо» при стоящей ленте — молчать нельзя.
    # Но и «сбой» это не всегда: на выходных arXiv не объявляет ничего, а пятничные
    # работы доезжают к понедельнику. Отдельный код 3 = «пусто, и это может быть
    # нормально»; конвейер такой день помечает пустым, а не красит красным
    # (владелец 30.08: два пайплайна должны проходить БЕЗ ошибок).
    if made == -1:
        EXIT_CODE = 3
...
def _build_derived_assets():
    ...
    B42_SKIP_DERIVED=1 — для МАССОВЫХ прогонов, где run.py зовут в цикле по одной работе.
    Здесь шесть шагов, и первый из них перестраивает поисковый индекс по всему корпусу:
    на трёхстах вызовах это триста пересборок, каждая из которых выбрасывается следующей.
    Оркестратор (tools/topics.py, дотяжка темы) ставит флаг на куски и снимает в конце,
    когда пересобрать надо ровно один раз. Одиночный вызов флага не знает и работает
    как раньше."""
    if _is_readonly_command() or os.environ.get("B42_SKIP_DERIVED") == "1":
        return
...
def _publish_to_r2():
    ...
    # SKIP_R2_PUBLISH убран 2026-08-06 (решение ведущей). Он делал ровно то же, что
    # B42_NO_PUBLISH, нигде не был описан и жил тихо — а второй, никому не известный способ
    # отключить публикацию опаснее отсутствия первого: однажды кто-то ставит его в скрипте,
    # и выкладка молча перестаёт происходить. Способ выключить публикацию теперь один
    # на разовый запуск (B42_NO_PUBLISH=1) и один на сессию (файл .no-publish).
    ...
    # Дев-режим двумя способами, и это не дублирование. Переменная окружения защищает ОДНУ
    # команду: поставил её на сборку — и забыл на следующей, а следующая опубликовала.
    # Ровно так 2026-08-06 свежий гид уехал на прод сам: сборка шла с B42_NO_PUBLISH=1,
    # а догон и reindex после неё — уже без него. Ошибки в коде не было, была ловушка:
    ...
    if os.environ.get("B42_NO_PUBLISH") == "1" or marker.exists():
        ...
        print(f"\n{'=' * 60}\n⏸️  публикация ПРОПУЩЕНА: {why} (дев-режим)\n"
              ...
              + (f"\n   Снять режим: удалить {marker.name}" if marker.exists() else "")
              + f"\n{'=' * 60}")
        return
