...
PACK_TIMEOUT = 90                  # сколько ждать ГРУППУ пачек, прежде чем бросить.
                                   # Пачка считается 2-5 секунд. Полторы минуты — уже
                                   # с запасом; зависшие соединения к поставщику
                                   # случаются регулярно, и цена ожидания важнее
                                   # цены пересчёта. Брошенная пачка не теряется:
                                   # её доберёт следующий запуск, в .ids она не попала.
    ...
    for m in ms:
        ...
        for g in range(0, len(packs), WORKERS):
            ...
            try:
                ...
                # Ограничение на ГРУППУ целиком, а не на каждый поток по очереди.
                # В первой версии было `fu.result(timeout=...)` в цикле, и потолок
                # умножался на число потоков: шесть зависших подряд держали прогон
                # полчаса вместо четырёх минут. Здесь ждём всех разом и забираем то,
                # что успело; остальное доберёт следующий запуск.
                ready, late = cf.wait(futs, timeout=PACK_TIMEOUT)
                if late:
                    print(f"  !! {m}: {len(late)} пачек зависли дольше {PACK_TIMEOUT}с — "
                          f"брошены, доберутся следующим запуском")
                res = []
                for fu in ready:
                    try:
