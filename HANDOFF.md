...
- **⚠️ Известный непочиненный баг** (не блокирует прод, но опасен при работе со справочниками):
  `tag_describe.py`/`law_describe.py` могут затереть накопленные графом связи тег/закон↔учёный
  при любом запуске (даже `--only`). Подробности и правильный фикс — TODO.md, раздел «🔴 2026-07-18».
  **Не запускать эти скрипты, пока не почищено.**
...
### Известный непочиненный баг (см. §0 выше)
- `tag_describe.py`/`law_describe.py`: `main()` безусловно перезаписывает graph-связи
  тег/закон↔учёный для ВСЕГО реестра при любом запуске, затирая накопленное `run.py evolve`.
  Уже разово обрушивало `tags-graph.json` (1533→259 связей), данные восстановлены из git.
  **Не запускать эти скрипты до фикса** — подробности в TODO.md «🔴 2026-07-18».
