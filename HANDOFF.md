> ⚠️ **АРХИВ (заморожен 2026-08-05, проверено 2026-09-01).** Документ содержит
> устаревшие и местами ОПАСНЫЕ указания (например, `run.py html` без флагов публикует
> на прод) и описывает проект вдвое меньшего размера: на момент заморозки было 2 160
> работ, сейчас 6 749; двух конвейеров, машины знаний из 3 613 понятий и замков от
> гонок тогда не существовало.
>
> **Актуальные точки входа:** **задачи/СТАТУС.md** (где мы, обновлён 1 сентября) ·
> **ДЛЯ-АУДИТОРА.md** (полное досье, обновлено 1 сентября) · **задачи/КОНЦЕПТ.md**
> (кто чем владеет) · **ПРАВИЛА-РАБОТЫ.md** (как работаем).
>
> Находка внешнего аудита: новичок, начавший отсюда, получал советы против
> действующих правил. Файл оставлен как история решений, а не как инструкция.

# bridge42worlds — Project Handoff

> **Работаете не один?** Порядок работы в команде — [ПРАВИЛА-РАБОТЫ.md](%D0%9F%D0%A0%D0%90%D0%92%D0%98%D0%9B%D0%90-%D0%A0%D0%90%D0%91%D0%9E%D0%A2%D0%AB.md): в ветках только исходники, сборку делает один.
> **Что делать прямо сейчас** — [ЗАДАЧИ-СЕЙЧАС.md](%D0%97%D0%90%D0%94%D0%90%D0%A7%D0%98-%D0%A1%D0%95%D0%99%D0%A7%D0%90%D0%A1.md): задачи по ролям (DevOps, разработчик, контент).
...
- размеченные **понятиями**, **учёными** и **авторами**;
- с извлечёнными из PDF картинками, подписями и AI-обложкой (3:2, FLUX-2-pro).

Вокруг статей выстроен **единый граф знаний**: понятия⇄формулы⇄учёные и их
взаимосвязи, чтобы читатель мог гулять по знанию, а не только по ленте.

**Понятие — одна сущность на всё.** До 24 августа тег и закон были разными
предметами с разными страницами; теперь это один реестр (`data/concepts-live.json`,
около 3600 записей), а `kind` говорит, что именно перед нами: закон, величина,
константа, явление, прибор, метод. Старые адреса `/tags/…` и `/laws/…` отдают
переадресацию на `/concepts/…`. В интерфейсе слова «тег» нет нигде.
...
- Полнотекстовый поиск + фильтры `#понятие @автор !учёный`.
- Страницы понятий, формул, учёных, авторов — каждая со списком релевантных статей,
  живой мини-картой своего окружения и AI-обложкой.
- Страница понятия: карточка, разбор формулы (что означает каждый символ, со
  значениями констант), области на карте знания, связи по существу, статьи.
- Страница автора дополнительно: его понятия и работы, всё динамикой из облака.
- Страница авторов: **алфавитный указатель A–Z** с per-letter страницами + теги-чипсы
  в строках авторов (кликабельные, с тултипами).
- Порядок в меню: `main | concepts | scientists | sections`.
- **Граф знаний** (`/concepts/graph.html`): понятия, формулы и учёные одним полотном.
  Три кадра — обзор областей, область целиком, окружение одного понятия; входит по
  адресу (`?focus=`, `?set=`, `?group=`), поэтому переход с карточки открывает то,
  ради чего пришли. Кадры собираются ЗАПРОСОМ в D1, а не пересчитываются заранее.
- **Связи трёх происхождений.** Статьи дают соседство (встречаются вместе), вектор —
  похожесть карточек, знание — названное отношение («входит в», «описывает»,
  «измеряется в»), которое находит `tools/link_weaving.py`. Третий источник нужен
  потому, что закон и константу статьи почти никогда не называют рядом, а связаны
  они неразрывно.
- **Области названы человеческим языком** (`tools/group_names.py`): не «группа 25»
  и не склейка трёх участников, а «Гидродинамика и турбулентность» с пояснением.
- Облако понятий: 50 областей, внутри — состав со счётчиками статей.
...
lang/
  ru/                                  ← язык-источник (default_lang)
    ...
    data/
      # ⚠️ Справочники ниже — ПРЕЖНЕЕ устройство (тег и закон как разные предметы).
      # Они живы и остаются источником текстов, но сайт с 24 августа строится из
      # единого реестра понятий data/concepts-live.json; /tags/ и /laws/ отдают
      # переадресацию. Новую разметку и новые страницы делай через реестр понятий.
...
data/
  concepts-live.json     ← ★ РЕЕСТР ПОНЯТИЙ: ~3600 записей (kind, имена на 5 языках,
                            карточка, статьи, соседи, формулы, области). Главный файл
                            сегодняшнего сайта: из него страницы /concepts/, облако,
                            граф и таблицы D1.
  concept-links-knowledge.json ← связи по существу, найденные знанием (part_of,
                            describes, measures…) — третий источник рёбер графа
  group-names.json       ← человеческие имена 50 областей + пояснения
  concepts-graph.json    ← выгрузка графа: узлы понятий/формул/учёных, рёбра
                            четырьмя элементами [a, b, вес, происхождение связи]
  knowledge-graph.json   ← ПРЕЖНИЙ граф: узлы t:/l:/s:, рёбра tag-tag/law-law/sci-sci/
                            law-tag/sci-tag/law-sci (питал старый /graph/)
