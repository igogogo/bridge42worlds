...
def main():
    ...
    # Понятия БЕЗ ВЕКТОРА этот шаг обслужить не может — и раньше молчал о них
    # вовсе: условие «c in pos» выкидывало их из счёта, и вывод «без соседей: 0»
    # означал не «все связаны», а «все, кого я вижу, связаны». Сегодня таким
    # оказался закон сохранения импульса: ни статей, ни соседей, ни формул — и
    # никакого сигнала. Пока их единицы, это мелочь; после большого рождения
    # понятий так же тихо осиротеет полсотни. Поэтому говорим вслух.
    no_vec = [c for c, v in live.items()
              if c not in pos and not (v.get("related") or [])]
    print(f"понятий без соседей: {len(lonely)} из {len(live)}")
    if no_vec:
        print(f"⚠ без вектора и потому необслуживаемых: {len(no_vec)} "
              f"({', '.join(no_vec[:5])}{'…' if len(no_vec) > 5 else ''})")
        print("  им нужен пересчёт векторов карточек (concepts_super --embed)")
    if not lonely:
        return 0
