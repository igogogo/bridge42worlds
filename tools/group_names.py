...
def ask(batch, key):
    ...
    if isinstance(got, dict):
        for v in got.values():
            ...
            # ОДНА ГРУППА В ПАЧКЕ — И ОТВЕТ ДРУГОЙ ФОРМЫ. На пачке из нескольких моделью
            # возвращается список, и разбор ждал только его. Но когда доназвать надо
            # ровно одну группу (а это обычный случай: появилась новая область, или
            # старую переименовываем), ответ приходит одним объектом — список не
            # находится, цикл ниже проходит по пустоте, и запрос молча пропадает.
            # Инструмент при этом бодро печатает «названий: 49» и ничего не меняет
            # (поймано 10.09 на климатической группе). Одиночный объект принимаем как
            # список из одного.
            if "name_en" in got or "name_ru" in got:
                got = [dict(got, n=got.get("n", 1))]
    out = {}
