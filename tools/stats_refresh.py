...
def main():
    ...
    #
    # Владелец 24.08: «увидели лимит на 40 минут, всё время падает». 2400с хватало, пока
    # сайт был меньше — теперь 249к+ файлов, и после дня с правкой шаблона (полная
    # пересборка) дельта здесь разовая и большая: deploy_r2 сверяет хэши по ВСЕМ файлам,
    # а не только по тем немногим, что должна была выложить эта задача. Обычный день
    # укладывается в минуты; редкий «после полной пересборки» — то, под что и нужен запас.
    if not args.dry:
        code, tail = run("cloudflare/deploy_r2.py", timeout=7200)
        if code == 0:
            print("  ✅ выложено")
        else:
