...
# Сколько сильнейших связей держит понятие. Теперь это ПОТОЛОК, а не пожелание:
# связь остаётся, только если попала в топ обоих концов (см. ниже). Шестнадцать, а не
# двенадцать: при взаимном отборе выживает заметно меньше, и на двенадцати половина
# понятий оставалась с тремя связями.
TOP_PER_NODE = 16
# Ниже скольких связей понятие считается недосвязанным и добирает смысловые. Раньше
# добирали только тех, у кого связей НЕТ ВОВСЕ; после перехода на взаимный топ таких
# «почти одиноких» стало семь сотен, и страница понятия у них выглядела пустой.
THIN_DEGREE = 3
...
def main():
    ...
    # ВЗАИМНЫЙ ТОП: связь остаётся, только если она в числе сильнейших У ОБОИХ понятий.
    #
    # Прежде хватало попадания в топ одного конца — и объединение таких списков давало
    # ровно то, чего правило обещало избежать. Обещание «топ-12 на узел» не выполнялось
    # никогда: у schwarzschild_metric оказалось 163 связи, у enso — 88. Хаб попадает в
    # топ десятков чужих списков, и каждое такое попадание добавляет ему ребро.
    #
    # Виднее всего это стало на климатической части (владелец 10.09: «там всё связано со
    # всем и теряется смысл»). Причина не в ней самой: 225 работ про Эль-Ниньо — корпус
    # однотемный, и совместная встречаемость там ничего не различает. Термин
    # spring_predictability_barrier стоит в 195 работах из 225, multivariate_enso_index —
    # в 192. Любая пара таких понятий набирает сотню общих статей просто потому, что все
    # работы про одно. Порог «две общих статьи» на таком корпусе не значит ничего.
    #
    # Взаимность лечит и то, и другое разом: у хаба связей теперь не больше TOP_PER_NODE,
    # а связь между двумя однотемными понятиями выживает, только если каждое из них
    # действительно считает другое одним из ближайших. Климатическая группа осела с 21%
    # возможных пар до 5%, наибольшая степень в графе — со 163 до 16.
    ...
    for a in per_node:
        per_node[a].sort(reverse=True)
    top = {a: {b for _n, b in lst[:TOP_PER_NODE]} for a, lst in per_node.items()}
    keep = set()
    for a, lst in per_node.items():
        for _n, b in lst[:TOP_PER_NODE]:
            if a in top.get(b, ()):
    ...
    for cid, v in live.items():
        i = idx[cid]
        # Условие — «связей меньше трёх», а не «нет статей». Ксенон, эффект
        ...
        # нет — точка без рёбер одинаково не находится. А со взаимным топом к ним
        # добавились почти одинокие: связь есть, но одна, и страница понятия пуста.
        if degree[i] >= THIN_DEGREE:
            continue
