...
def by_paper(aid, lang, to, send, test=False, lang_explicit=False, over_cap=False):
    ...
    if council_mail.send(to, subj, body, sender=FROM, html=html):
        if test:
            ...
            # Номер работы обязателен: без него письмо не привязать к работе, и на
            # доске посещений не видно, к кому мы уже обращались. Все 35 писем до
            # 10.09 записаны с пустым номером именно из-за этой строки — здесь его
            # просто забыли передать, хотя работа известна (art["id"] выше).
            remember(who, to, lang, art["id"])
...
def main():
    ...
    ap.add_argument("--test", action="store_true",
                    help="пробное письмо СЕБЕ: обязателен --to, в журнал не пишется")
    a = ap.parse_args()

    # --test БЕЗ --to УХОДИЛ НАСТОЯЩЕМУ АВТОРУ. Ключ задумывался как «покажи, как
    # выглядит письмо, на живой работе» — и в журнал поэтому не пишет, чтобы автор,
    # на котором проверяли вид, не попал навсегда в «уже написали». Но адрес по
    # умолчанию берётся из самой работы, и без --to пробный прогон слал письмо
    # человеку: мимо проверки «уже писали» и мимо суточной нормы разгона. Поймано
    # 13.09.2026 своей же проверкой связи — письмо ушло автору 2608.30377v1.
    # Адрес получателя при пробе обязан быть назван вслух.
    if a.test and not a.to:
        print("⛔ --test требует --to: без адреса пробное письмо уходит настоящему")
        print("   автору, не записываясь в журнал, — и он может получить второе.")
        return 2
