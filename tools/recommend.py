...
def build(aid, show=False, hints=""):
    ...
    # ── Этический фильтр ДО записи ───────────────────────────────────────────────
    # output_guard написали вместе с ролью рекомендаций — и не подключили никуда:
    # аудит 16 августа нашёл, что grep по import пуст, а блок уже стоит на 612 статьях.
    # Подключаем в единственно правильном месте — до того, как черновик коснулся диска
    # (порядок операций и есть часть требования роли: сохранённый до проверки черновик
    # с критикой уже нарушил правило «список слабых мест не записывается никуда»).
    #
    # Дешёвые проверки (стоп-обороты, вопросы к автору) — всегда; смысловые (flawless,
    # coverage) требуют ключа и денег, их зовёт сам guard при наличии ключа в среде.
    # Провал НЕ роняет прогон: раздел просто не записывается у этой статьи, и очередь
    # вернётся к ней следующим прогоном — как с любым другим браком генерации.
    try:
        from output_guard import check_all
        draft_text = "\n\n".join(
            [out.get("seen", ""), out.get("strength", "")]
            + [x.get("text", "") for x in (out.get("directions") or [])]
        ...
        print("  ⚠️ output_guard не найден — записываю без проверки выхода")
