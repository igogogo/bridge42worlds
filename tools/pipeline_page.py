...
PHASES = [
    ...
    ("Панель El Niño", "источники → ряды и риски → тревоги → вердикт модели → журнал и лента",
     ["источники", "ряды и риски", "тревоги", "саммари модели", "снимок и история",
      "журнал значений", "лента новостей", "готово"]),
]
...
(function () {
  ...
  function KIND_NAME(run) {
    ...
    /* Третий род — прогон по теме (tools/enso/works_run.py): не день и не весь архив, а
       список работ одной темы кусками. Шёл мимо схемы, и трое суток разбора выглядели
       на ней как тишина (владелец 06.09). */
    if (k === "topic") return "по теме";
    /* Четвёртый род — обновление панели El Niño (tools/enso/refresh.py): источники,
       пересчёт, тревоги, саммари модели, журнал и лента (владелец 06.09). */
    if (k === "enso") return "панель El Niño";
