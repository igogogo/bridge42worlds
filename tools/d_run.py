...
def main():
    ...
    # ШАГА «КАДРЫ ГРАФА» БОЛЬШЕ НЕТ. Обзор и кадр области воркер собирает
    # запросом к таблицам областей — тем же способом, каким давно собирает
    # окружение понятия. Пересчитывать и заливать готовые JSON-кадры после каждой
    # правки реестра не нужно: данные обновились — кадр обновился (владелец
    # 28.08: «а разве это не динамика, зачем их обновлять?»). Сами таблицы
    # уезжают в предыдущем шаге вместе со всем реестром.
