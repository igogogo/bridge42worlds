...
def counts():
    ...
    try:
        ...
        out["узлов графа"] = len(g["nodes"])
        out["рёбер графа"] = len(g["edges"])
        out["рёбер по знанию"] = sum(1 for e in g["edges"] if len(e) > 3)
        out["групп с именем"] = sum(1 for x in g.get("groups") or []
                                    if (x.get("note_ru") or x.get("note_en")))
    except Exception:
        pass
    return out
...
def report():
    ...
    lines = ["# Ночь 27–28 августа: что сделано", "",
             ...
             "",
             "**Утром, по замечаниям владельца.** Карточка статьи в списках: текст "
             "снова обтекает миниатюру (обрезка описания в три строки создавала "
             "блочный контекст и обтекание убивала), кнопки уровней уехали вниз в "
             "строку с реакциями и больше не подсвечены — выбора-то не было. "
             "Вкладки на странице понятия убраны совсем: за ними пряталось 1503 "
             "знака из 1839. Мини-граф стал живым — узел тянется за курсором, — и "
             "занимает 90% ширины вместо 32%. Группы графа получили человеческие "
             "имена и строку «о чём область» вместо склейки трёх участников. "
             "Появился третий источник связей: не статьи и не вектор, а знание — "
             "закон и его константа связаны потому, что одна входит в другую.",
             "",
