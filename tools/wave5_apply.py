    ...
    if kfix.exists():
        for cid, kind in load(kfix).items():
            if cid in out and kind:
                out[cid]["kind"] = kind
    # СОСЕДИ ПО СМЫСЛУ — тем, у кого нет соседей по статьям. Супер считает связи
    # по со-встречаемости, и понятие без статей остаётся ни с чем: открывается,
    # показывает определение и никуда не ведёт. Такие связи считает
    # tools/vector_neighbors.py по вектору карточки; помечены src=vec, чтобы
    # страница и граф отличали их от статейных.
    vnb = ROOT / "data" / "concept-vec-neighbors.json"
    if vnb.exists():
        for cid, got in load(vnb).items():
            if cid in out and not out[cid]["related"]:
                out[cid]["related"] = got[:10]
