...
# Переразметки здесь БОЛЬШЕ НЕТ (владелец 07.09: «и да перенеси»). Её убрали из плана,
# но забыли отсюда, и шаг молча пропускался: план обещал, исполнитель говорил «недельный
# шаг, в обычном прогоне пропущен». Она местная, бесплатная и проходит архив за 5 секунд.
WEEKLY_ONLY = {
...
def main():
    ...
    # ПЕРЕРАЗМЕТКА АРХИВА СТАЛА ЕЖЕДНЕВНОЙ (владелец 08.09). Она держалась недельной
    # из осторожности, а по стоимости давно дневная: вектор считается локально, модель
    # не зовётся, весь архив в 6428 статей прошёл за минуту. Недельная задержка означала,
    # что новое понятие неделю не дотягивается до старых статей — при 127 рождённых за
    # два дня это заметная потеря связности.
    weekly_only = ["g-grow", "f-support", "tr-formulas", "super", "vecnb",
    ...
    st["plan"] = [f"day-{d}" for d in days] + ["biorxiv"] + _plan
    save(st)
    ...
    # ── I-бис. БИОАРХИВ ──────────────────────────────────────────────────────
    # Владелец 12.09: «меня интересует тот источник биотех архив по насыщению, по
    # работам, связанным с омоложением», и следом «бери 8».
    #
    # ЗАЧЕМ ЗДЕСЬ, А НЕ ОТДЕЛЬНОЙ ЗАДАЧЕЙ. Работа, принятая после этой точки, не
    # успела бы на насыщение знаниями: ни разметки, ни понятий, ни графа, ни
    # страницы — всё это идёт ниже по цепочке и берёт то, что уже в архиве. Поэтому
    # приём стоит сразу за днями arXiv: дальше био-работы идут общим потоком и
    # ничем от arXiv-работ не отличаются.
    #
    # Почему у нас вообще нет словаря старения: в корпусе q-bio всего 3,3%, а
    # клеточной биологии девять работ. Реестр понятий растёт ИЗ разобранных статей,
    # и сочинять словарь руками нельзя — получим термины, под которыми нет работ.
    # Насыщение идёт отсюда.
    #
    # Мягкий шаг: у препринт-сервера свои выходные и свои простои, и молчание
    # биоархива не повод ронять дневной прогон arXiv.
    run("biorxiv", [PY, "tools/outside/biorxiv.py", "--days", "1",
                    "--limit", "8", "--apply", "--generate"],
        timeout=4 * 3600, soft=True, env=DAY_ENV)
    ...
    # Обычный прогон собирает только новое и только два языка: он обязан укладываться
    # в минуты. Недельный собирает ВСЁ и на ВСЕХ языках.
    #
    # Сужение «ru,en» до 08.09 стояло безусловно, то есть и в недельном тоже, — прямо
    # против его назначения, описанного строкой ниже. Нашлось на переразметке архива:
    # ru и en получили новые пометки понятий той же ночью, а es, ar и fr остались со
    # вчерашними, потому что их страницы никто не пересобирал. Три читателя из пяти
    # видели вчерашнее знание и никакой ошибки при этом не возникало.
    env = {} if FULL else {"B42_LANGS": "ru,en", "B42_ONLY_NEW": "1"}
    if a.no_publish:
