...
NUMBERS = {
    ...
    "vec-ours": [("статей в выгрузке", r"выгружено: (\d+)"),
                 ("стоимость эмбеддинга", r"\$([\d.]+)")],
    "vec-push": [("векторов обновлено", r"смысловой индекс обновлён: (\d+)")],
...
def main():
    ...
    # ДОТЯЖКИ ИЗ «ПРОСТО» ЗДЕСЬ БОЛЬШЕ НЕТ — И ЭТО ГЛАВНОЕ, ЧТО НАДО ЗНАТЬ ПРО ЭТО МЕСТО.
    #
    # Стоял шаг tools/express_uplift.py: он брал «Просто» (написанное по авторской
    # аннотации) и поднимал его до «Популярно». Уровень получался настоящий по тону и
    # пустой по знанию — под ним всё та же аннотация, а не работа. За месяц так поднято
    # 2 090 статей: 1 532 пятого августа и по 70-80 ежедневно до 24-го.
    #
    # Владелец 2026-09-01, когда цепочку разобрали вслух: «популярно из просто не делаем».
    # Направление одно — «Подробно» из текста PDF, «Популярно» из «Подробно». Настоящая
    # дотяжка живёт отдельно (tools/deep_uplift.py) и идёт своим ночным заходом в дешёвое
    # окно: она тяжелее дневного прогона и не должна с ним толкаться за замок дерева.
    #
    # Те 2 090 стоят в очереди на пересборку первыми — они выглядят готовыми, и потому
    # опаснее прямо заблокированных.
    ...
    # ВЕКТОРА СТАТЕЙ — ОТДЕЛЬНО ОТ ВЕКТОРОВ ПОНЯТИЙ, и в конвейере их не было вовсе.
    # Шаг выше обновляет пространство «concepts» (карточки понятий) и отчитывается
    # успехом — а смысловой индекс статей («ours») никто не догонял: 07.09 в нём лежало
    # 7086 работ при 7147 в корпусе, то есть весь свежий день читателю по смыслу не
    # находился. Словами находился (это D1), и потому дыра не бросалась в глаза.
    # Выгрузка считает только изменившиеся тексты, заливка — только изменившиеся вектора;
    # шестьдесят статей стоили $0.014 и полторы минуты.
    run("vec-ours", [PY, "embeddings_export.py"], timeout=3600, soft=True)
    run("vec-push", [PY, "cloudflare/vector_build.py"], timeout=2 * 3600, soft=True)
