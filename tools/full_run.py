...
def main():
    ...
    # ему и место — рядом с рождением этих самых экспрессов.
    #
    # ПЕРЕВОД НА ВСЕ ЧЕТЫРЕ, А НЕ ТОЛЬКО НА АРАБСКИЙ. Замер 01.09 по архиву: честное
    # «Популярно» есть у 34% экспрессов по-русски, 20% по-арабски и 1-3% на английском,
    # испанском и французском. Арабский был первым по решению владельца и ушёл вперёд;
    # остальные три языка стоят почти на нуле, и поднимать русский без них значит
    # растить перекос. Поднятое переводится тем же проходом на все четыре.
    run("uplift", [PY, "tools/express_uplift.py", "--pilot", "20",
                   "--translate", "en,es,ar,fr"], timeout=3 * 3600, soft=True)
