...
FILES = ["data/enso/latest.json", "data/enso/history.json", "data/enso/glossary.json", "data/enso/journal.json",
         ...
         # «against analogues» грузит клима бокса прямо из браузера, а выкладка их не отправляла.
...
FRESH_FILES = ["data/enso/fresh.json", "data/enso/ops.json", "data/enso/runs.json", "data/enso/planet.json", "data/enso/cities.json", "data/enso/fires.json", "data/enso/water.json", "data/enso/futures.json", "data/enso/ice-snow.json", "data/enso/glaciers.json",
...
def push_index(no_index, why):
    ...
    # ОБЛАКА ПОНЯТИЙ У ЯКОРЕЙ — второй наш слой для панели, и он отставал молча.
    # Индекс чата пересобирался здесь с самого начала, а concepts.json — нигде: на 10.09
    # он был от 08.09, хотя реестр понятий с тех пор менялся дважды. Панель показывала
