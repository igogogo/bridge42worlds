...
#
# Цены DeepInfra за миллион токенов. ЭТО ОЦЕНКА ПОРЯДКА, а не сверенный прайс: точные
# числа нужно взять из счёта DeepInfra и подставить сюда. Даже грубая оценка ближе к
# правде, чем прежний расчёт по DeepSeek, — но пока цифры не сверены, помечаем это
# прямо здесь, чтобы никто не считал их проверенными.
