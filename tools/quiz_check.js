...
const problems = [];
// Предупреждения: подозрительное, где решает человек, а не машина (см. checkBareOrdinal).
const warnings = [];
...
/* ГОЛЫЕ ПОРЯДКОВЫЕ. Правила выше требуют рядом с порядковым слово «вариант/option/opción».
 * Из-за этого требования проверка 18 августа показала 4 места, а на деле их было десять:
 * «Четвёртый — самая частая ошибка», «La cuarta es el error», «الرابعة هي الخطأ» проходили
 * молча — существительного рядом нет. Хуже: невидимым оставался РУССКИЙ ИСТОЧНИК, а он и есть
 * причина. Переводчик честно переносил позиционную ссылку из оригинала, поэтому три круга
 * борьбы с промптом ничего не могли дать. Ловим голую форму: она законна перед
 * существительным («Второе начало», «Третий закон») и подозрительна перед тире или глаголом.
 * Это ПРЕДУПРЕЖДЕНИЯ: решает человек, машина только показывает. */
const OK_AFTER = new RegExp('^(начал|закон|постулат|принцип|правил|уравнен|механизм|способ|путь|случа|шаг|слагаем|член|производн|космическ|по|из|law|principle|rule|term|step|mechanism|case|ley|principio|regla|mecanismo|caso|paso)', 'i');
const BARE = [
    new RegExp('(?:^|[.;:!?»]\s+)(перв|втор|трет|четв[её]рт|пят|последн)(?:ый|ий|ая|ое|ья|ье|ой)\s+(\S+)', 'i'),
    new RegExp('(?:^|[.;:!?"]\s+)the\s+(first|second|third|fourth|fifth|last)\s+(\S+)', 'i'),
    new RegExp('(?:^|[.;:!?»]\s+)(?:la|el)\s+(?:primer|segund|tercer|cuart|quint|[uú]ltim)[oa]\s+(\S+)', 'i'),
    new RegExp('(?:الأول|الثاني|الثالث|الرابع|الخامس|الأخير)(?:ة)?\s+(هي|هو|غير|لا|ليس)'),
];
function checkBareOrdinal(where, q) {
    if (q.type !== 'mcq') return;
    ['why', 'hint'].forEach(field => {
        ...
        for (const re of BARE) {
            const m = re.exec(text);
            if (!m) continue;
            const next = (m[2] || m[1] || '').replace(/^[«"']/, '');
            if (next && OK_AFTER.test(next)) continue;
            warnings.push(`${where}: в «${field}» голое порядковое «${m[0].trim().slice(0, 44)}» — `
                + `если это ссылка на вариант, читатель увидит его на другом месте; `
                + `если на закон или механизм, всё в порядке`);
            return;
        }
    });
}
