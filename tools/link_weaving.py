...
# Кто вправе «описывать»: у описания есть автор — теория, закон, уравнение, эффект,
# принцип, модель. Всё остальное описывают, а не описывает: явление и объект —
# очевидно, но и величина с прибором тоже («электронная плотность описывает эффект
# Фарадея», «светоделитель описывает полное внутреннее отражение» — обе неправда).
# «concept» оставлен вправе описывать: под ним в реестре лежат и теории тоже —
# класс общий, и запрещать ему значило бы выбросить верное вместе с неверным.
_DESCRIBERS = {"law", "equation", "effect", "theorem", "theory", "model", "principle",
               "concept", "method", "statistics"}
# Кто описывать не может — что бы ни стояло справа.
_NOT_DESCRIBERS = {"phenomenon", "object", "substance", "process", "quantity",
                   "instrument", "property", "constant"}
...
def fix_directions(got, live):
    ...
    for cid, links in got.items():
        ...
        for lk in links:
            ...
            # «Описывает» может только теория, закон, уравнение, эффект. Явление,
            # объект, вещество и процесс не описывают — их описывают. Выборка из
            # двенадцати таких связей не дала ни одной верной: «катод описывает
            # электрическое поле», «энергосистема описывает электрическое поле».
            # Дальше два разных случая, и валить их в один нельзя:
            #   цель — закон/уравнение/эффект: пара верная, направление обратное
            #     («уравнение Планка — Эйнштейна описывает микроволновый фотон»);
            ...
            if lk.get("rel") == "describes" and v.get("kind") in _NOT_DESCRIBERS:
                if w.get("kind") in _DESCRIBERS:
                    out.setdefault(to, []).append(
                        {"to": cid, "rel": "describes", "w": lk.get("w", 2)})
                else:
                    keep.append({"to": to, "rel": "same_area", "w": lk.get("w", 2)})
                n += 1
                continue
