...
def next_id(date_str):
    """Свободный номер вида YYMM.9NNNN для месяца этой работы.

    Номер свой, из девятого десятка тысяч: формат YYMM.NNNNN нужен генератору (из номера
    выводится месяц), а до 90000 arXiv не доходит — там около двадцати тысяч работ в
    месяц даже в самые плотные годы. По номеру сразу видно, что работа наша.

    БЫЛО 900NN, ТО ЕСТЬ ДЕВЯНОСТО ДЕВЯТЬ НА МЕСЯЦ. Пока работы приходили поштучно, этого
    хватало с запасом. Первый же большой забор съел 75 из 99 за август, а следующий пул
    той же недели содержит 92 работы — они бы не влезли, и приём упёрся бы ПОСРЕДИ
    прогона, потеряв половину. Причём номер считается по дате РАБОТЫ, а не по дате
    разбора, так что разложить нагрузку по дням нельзя: неделя августа — это август.

    Новая схема включает старую: 9{1:04d} даёт 90001, ровно как 900{1:02d}. Уже
    заведённые номера остаются валидными, переименовывать нечего.
    """
    yymm = date_str[2:4] + date_str[5:7]
    used = set()
    for p in WORKS.glob(f"{yymm}.9*.json"):
        used.add(p.stem)
    for p in (ROOT / "lang" / "ru" / "archive").glob(f"*/{yymm}.9*"):
        used.add(p.name)
    for n in range(1, 10000):
        aid = f"{yymm}.9{n:04d}"
        if aid not in used:
            return aid
    raise SystemExit(f"⛔ в месяце {yymm} кончились свои номера (90001–99999)")
