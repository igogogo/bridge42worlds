...
// Проверяем все три файла темы — обзор (course.json), методичку (guide.json) и параграфы —
// и во всех языковых ветках: ссылки на сайт лежат в каждой отдельно, поэтому «зелено
// по-русски» ничего не говорит о том, что видит читатель на арабском.
...
// Правда об адресе — только каталог отрендеренных страниц. Каталоги языков совпадают
// (разница ru и en — служебный audit.html), поэтому хватает одного, русского.
// Граф знаний источником адресов быть НЕ может, и это не вкусовщина: имена учёных в нём
// живут в человеческом виде («s:Albert A. Michelson»), а страницу сайт называет через
// author_slug — пробел в подчёркивание, точка ВОН (Albert_A_Michelson.html). Если сложить
// имена из графа в каталог, приведя только пробелы, в нём заводятся девять адресов вида
// Albert_A._Michelson, которых на диске нет никогда. Ровно такой id в course.json проверка
// пропускала молча, а читатель получал 404 — та самая беда, ради которой каталог и заведён.
// Поэтому граф идёт в дело только там, где страницы ещё не отрендерены: лучше сверять с ним,
// чем не сверять ни с чем.
let KNOWN = null;
if (fs.existsSync(SITE)) {
    ...
    try {
        ...
        const fromGraph = { tag: new Set(), law: new Set(), sci: new Set() };
        (g.nodes || []).forEach(n => {
            const i = String(n.id).indexOf(':');
            if (i > 0 && fromGraph[n.kind]) fromGraph[n.kind].add(String(n.id).slice(i + 1).replace(/\s+/g, '_'));
        });
        Object.keys(KNOWN).forEach(kind => { if (!KNOWN[kind].size) KNOWN[kind] = fromGraph[kind]; });
    } catch (e) { /* графа рядом нет — хватит каталога страниц */ }
    if (!KNOWN.tag.size && !KNOWN.law.size && !KNOWN.sci.size) KNOWN = null;
}
// Молчаливый пропуск — то же самое «зелено, а не проверено», поэтому говорим вслух.
if (!KNOWN) warn.push('справочников сайта рядом нет — ссылки на теги/законы/учёных НЕ проверены');
...
// Проверка идёт по учебному плану, а не по диску, поэтому папка темы, которой в плане нет,
// не проверяется ВООБЩЕ и об этом никто не узнаёт: отчёт зелёный, тема сырая. Так сейчас
// живут realgas и rigidbody. Ошибкой это не считаем (тему пишут прямо сейчас), но говорим.
fs.readdirSync(C).forEach(n => {
    if (!fs.statSync(C + '/' + n).isDirectory() || topicsInTree.includes(n)) return;
    warn.push(`тема ${n} лежит на диске, но её нет в учебном плане index.json — она не проверяется`);
});
...
topicsInTree.forEach(tid => {
    ...
    // Проверяли только параграфы, поэтому в обзорах спокойно жили id, которых на сайте
