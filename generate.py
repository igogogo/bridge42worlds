...
def entity_article_card(a, lang):
    ...
    #
    # НИ ОДИН не подсвечен. Здесь три равнозначных входа в статью — «открыть проще,
    # популярнее, подробнее», — а не переключатель состояния: сам текст карточки от
    # них не меняется. Подсветка обещала выбор, которого читатель не делал: владелец
    # 02.08 на карточках ленты («кажется, что "Просто" уже выбрано, а результат мы
    # видим только после перехода») и он же 28.08 здесь, на карточках справочников
    # («если у нас в списке пока ничего не выбрано, зачем кнопка "популярно"
    # подсвечена»). Клиентская лента это уже чинила, серверная карточка повторила
    # ошибку. На СТРАНИЦЕ статьи активный уровень подсвечивается по-прежнему — там
    # он и правда выбран.
    levels = level_switch_links(lang, None, a["date"], a["id"])
    ...
    return (f'<article class="article-card">'
            ...
            # УРОВНИ УЕХАЛИ ВНИЗ, в строку действий (владелец 28.08: «просто ·
            # популярно · подробно разместить внизу в той же строке, что и лайки,
            # выровнять по правому краю, чуть поменьше — сверху маячат над
            # названием»). Сверху они и правда спорили с заголовком: первое, что
            # видел глаз в карточке, был ряд кнопок, а не название статьи.
            ...
            # уровни — здесь же, правее лайков (CSS прижимает их к краю)
            f'{levels}'
            f'</div></div></article>')
...
def update_index(scipop, article, date_str, lang, version, abstract=""):
    ...
    idx.append({
        ...
        "thumbs": article.get("thumbs", 0),
        # ВСЕ АВТОРЫ, без потолка. Пятьдесят ставились ради веса индекса, а платили этим
        # люди: у работы 2511.14407 авторов 326, и 276 из них не получали ни строки в
        # облаке — страница есть, а списка работ и статистики нет (владелец 31.08:
        # «авторы все авторы»). Замер: снятие потолка удваивает число упоминаний
        # (47 953 → 94 897) и добавляет индексу 0,8 МБ из 12,6.
        "authors": article.get("authors", []), "date": date_str,
...
def update_all_authors(only=None):
    ...
    Зачем точечно. В графе 46 832 автора, и каждый прогон переписывал всех — на
    каждом языке. От нового дня меняются единицы: у шестнадцати статей примерно
    две-три сотни авторов, остальные сорок шесть тысяч перезаписывались тем же
    содержимым. Это минуты на ровном месте (28.08 именно здесь пришлось оборвать
    сборку), раздутая дельта выкладки и лишний обход в резервной копии.

    Списки-указатели по буквам и облако пишутся всегда: они дешёвые, а автор,
    добавленный сегодня, должен в них появиться.
    """
...
def build_signature():
    """Хэш того, что меняет БАЙТЫ страницы: шаблоны и генератор.

    CSS и JS сюда НЕ входят, и это осознанно. Страницы зовут их без версии в адресе
    (см. asset_ver: с 4 августа ?v= из шаблонов убрано), свежесть держат заголовки
    Worker — пять минут и 304. Значит правка стиля не меняет в готовой странице ни
    одного байта: пересобирать сорок тысяч файлов, чтобы получить те же сорок тысяч
    файлов, — чистая трата.

    Пока css/js были в подписи, каждая правка стиля обесценивала отпечатки всего
    архива. 31 августа это стоило конвейеру пяти часов: я правил css по ходу
    недельного прогона, и сборка честно перекладывала весь сайт (владелец: «сделай,
    чтобы при правке css так не менялось»).

    Что взамен: дисциплина, записанная в asset_ver — новый JS обязан работать со
    старой разметкой, а новый CSS не должен требовать нового класса, которого в
    выложенных страницах ещё нет. Окно рассинхрона — те же пять минут.
    """
    global _BUILD_SIG
...
def _load_fingerprints():
    ...
    try:
        ...
        if d.get("подпись") != build_signature():
            # Правка генератора или шаблона может изменить ЛЮБУЮ страницу, поэтому
            # честный ответ — пересобрать всё. Но в обычном прогоне это не то, чего
            # от него ждут: он должен добавить новые статьи за минуты, а не гнать
            # сорок тысяч страниц полтора часа (владелец 28.08: «чтобы не было
            # полных пересборок, если это не требуется, — только то, что новое»).
            # B42_ONLY_NEW=1 говорит: знаю про изменение, собери только новое.
            # Полная пересборка тогда — дело служебного прогона, и о ней сказано
            # вслух, а не забыто.
            if os.environ.get("B42_ONLY_NEW") == "1":
                print("   шаблоны или генератор изменились, НО собираем только новое "
                      "(B42_ONLY_NEW=1). Полная пересборка — tools/weekly_run.py")
...
def _index_entry(scipop, data, date_str, lang, version):
    ...
    return {
        ...
        "authors": data.get("authors", []), "date": date_str,   # все авторы, см. выше
        # Теги В ИНДЕКС — тоже из вектора. Я поправил показ на странице статьи и забыл
        # про индекс, а по нему живут лента, поиск и облако тегов: после полной пересборки
        # страницы были размечены по смыслу, а облако осталось прежним — 306 тегов вместо
        # 368 и те же 46% на десяти самых частых. Один источник правды, а не два.
