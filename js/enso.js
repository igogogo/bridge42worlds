/* Панель «El Niño 2026–2027»: один экран, без прокрутки страницы.
   ...
   Владелец 03.09: «дашборд должен быть одностраничным, там же есть в требованиях, без
   прокруток, это панель, на ней всё должно быть динамическое». Поэтому:

     · страница высотой ровно в экран; вниз ничего не уходит;
     · содержимое ПЕРЕКЛЮЧАЕТСЯ: вкладки сцены, переключатели внутри сцены, клик по риску
       открывает его ряд в главном окне, клик по строке моделей — её ошибки по выпускам;
     · графики рисуются под фактический размер своего окна и перерисовываются при любом
       изменении размера или темы (ResizeObserver + B42EnsoRedraw); никакой «резинки»;
     · длинные списки крутятся внутри своей плитки, а не страницей.

   Данные — три файла с нашего домена (latest.json, history.json, glossary.json). Сайт не
   пересобирается: обновить дашборд значит положить новый latest.json.
   Язык — английский (владелец 03.09), тексты генератора приходят уже английскими. */
(function () {
  'use strict';
  ...
  var T = {
    ...
    dockHint: 'Point at anything underlined: definition, source, date',
    whatThis: 'what this is', whyHere: 'why it matters here', source: 'source', dataDate: 'data date',
    aiBy: 'model summary', aiRules: 'rule-based digest', noModel: 'the model did not take part',
  ...
  var S = {                       // состояние панели: что показано и чем управляется
    D: null, G: {}, H: [],
    view: 'now', sub: {}, risk: null, model: null, series: 'sst_nino34', scenario: null,
  ...
  function srcSpan(payload, text) { return '<span data-src="' + esc(JSON.stringify(payload)) + '">' + esc(text) + '</span>'; }
  ...
  /* Легенда: справа, когда есть место; иначе строкой под заголовком. Так график остаётся
     читаемым и в узкой плитке телефона, и в широком окне. */
  function legendW(w) { return w < 560 ? 0 : Math.min(190, Math.round(w * 0.22)); }
  /* Когда легенда не помещается справа, она встаёт строкой под заголовком — значит, сверху
     нужно больше места, иначе легенда ложится на первую линию сетки (поймано на 375px). */
  ...
  function legend(items, w, h, R, top) {
    ...
    var x = 46;
    for (i = 0; i < items.length; i++) {
      s += '<rect x="' + x + '" y="' + (top - 8) + '" width="12" height="4" style="fill:' + items[i][1] + '"/>';
      s += '<text x="' + (x + 16) + '" y="' + (top - 4) + '" font-size="10">' + esc(items[i][0]) + '</text>';
  ...
  function chartRecent(w, W, H) {
    ...
    var s = svgOpen(W, H) + '<text class="tt" x="' + Lp + '" y="14">' + esc(w.label) + ' · anomaly to 1991–2020, last 400 days and two weeks ahead</text>';
  ...
  function chartAnalogs(N, W, H) {
    ...
    var Y = function (v) { return Tp + (vmax - v) / (vmax - vmin) * ph; };
    var s = svgOpen(W, H) + '<text class="tt" x="' + Lp + '" y="14">Niño 3.4 daily anomaly: ' + (N.year || '') + ' against the four strongest events, same calendar</text>';
    ...
    Object.keys(N.analogs).forEach(function (y) {
      ...
      leg.push([y + '→' + (parseInt(y, 10) + 1) + ': peak ' + fnum(a.peak), 'var(--a' + y + ')', 1.6]);
    });
    ...
    s += legend([[(N.year || 'now') + ' — now', 'var(--text)', 2.6]].concat(leg), W, H, R, Tp);
    return s + '</svg>';
  }
  ...
  function chartNoaa(NW, W, H) {
    ...
    var X = function (i) { return Lp + i / (n - 1) * pw; };
    var Y = function (v) { return Tp + (vmax - v) / (vmax - vmin) * ph; };
    var s = svgOpen(W, H) + '<text class="tt" x="' + Lp + '" y="14">NOAA weekly indices, last ' + n + ' weeks (anomaly, °C)</text>';
    s += gridY(vmin, vmax, .5, Y, Lp, R + 8, W, 1);
    ...
    s += legend(keys.map(function (k) { return [k[1] + ': ' + fnum(NW.latest[k[0]], 1), k[2], k[0] === 'n34a' ? 2.2 : 1.4]; }), W, H, R, Tp);
    return s + '</svg>';
  }
  ...
  function chartPlume(IRI, obs, W, H) {
    ...
    var Y = function (v) { return Tp + (vmax - v) / (vmax - vmin) * ph; };
    var s = svgOpen(W, H) + '<text class="tt" x="' + Lp + '" y="14">IRI model plume, ' + esc(IRI.issued) + ' issue: Niño 3.4 by season, °C</text>';
    s += gridY(vmin, vmax, .5, Y, Lp, R + 8, W, 1);
    ...
    Object.keys(models).forEach(function (name) {
      ...
      var hot = S.model === name;
      var col = hot ? 'var(--ochre)' : (m.section === 'dyn' ? 'var(--nina)' : 'var(--ok)');
      s += segs(fc.map(function (i) { return [X(i), fin(m.values[i]) ? Y(m.values[i]) : NaN]; }), col, hot ? 2.6 : 1, hot ? 1 : .4);
    });
    ...
    var leg = [['combined forecast', 'var(--text)', 3], ['previous issue' + (hist.length > 1 ? ' (' + hist[1].issued + ')' : ''), 'var(--soft)', 1.6, '5 4'],
               ['dynamical', 'var(--nina)', 1.4], ['statistical', 'var(--ok)', 1.4], ['reality this week', 'var(--nino)', 'dot']];
  ...
  function chartHistory(rows, W, H) {
    ...
    var s = svgOpen(W, H) + '<text class="tt" x="' + Lp + '" y="14">Our risk index by update, and the share of IRI models below reality</text>';
  ...
  function pacific(NW, W, H) {
    var Lp = 40, R = 16, Tp = 28, B = 26, pw = W - Lp - R, ph = H - Tp - B;
    var lon = function (d) { return Lp + (d - 120) / (290 - 120) * pw; };
    var lat = function (d) { return Tp + (15 - d) / 30 * ph; };
    var lv = NW.latest;
    var boxes = [['nino4', 'Niño 4', 160, 210, 5, -5, lv.n4a], ['nino34', 'Niño 3.4', 190, 240, 5, -5, lv.n34a],
                 ['nino3', 'Niño 3', 210, 270, 5, -5, lv.n3a], ['nino12', 'Niño 1+2', 270, 280, 0, -10, lv.n12a]];
    var s = svgOpen(W, H);
    ...
    boxes.forEach(function (b) {
      ...
      s += '<text x="' + (x + w / 2).toFixed(1) + '" y="' + (y + h / 2 + 12).toFixed(1) + '" text-anchor="middle" style="fill:' + col + '">' + fnum(v, 1) + '</text></g>';
    });
    return s + '</svg>';
  }
  ...
  function chartFood(FO, W, H) {
    ...
    var X = function (i) { return Lp + i / (n - 1) * pw; };
    var Y = function (v) { return Tp + (vmax - v) / (vmax - vmin) * ph; };
    var s = svgOpen(W, H) + '<text class="tt" x="' + Lp + '" y="14">FAO Food Price Index and groups, last ' + n + ' months (2014–16 = 100)</text>';
  ...
  function chartOverlay(ov, W, H) {
    ...
    var X = function (i) { return Lp + i / (n - 1) * pw; };
    var Y = function (v) { return Tp + (vmax - v) / (vmax - vmin) * ph; };
    var s = svgOpen(W, H) + '<text class="tt" x="' + Lp + '" y="14">Food price index as % of the onset month: current event against analogues</text>';
  ...
  /* Общий график ряда риска или ошибки модели: значения с датами, последняя точка подписана. */
  function chartMetric(m, W, H, title) {
    ...
    var X = function (i) { return Lp + i / (n - 1) * pw; };
    var Y = function (v) { return Tp + (vmax - v) / (vmax - vmin) * ph; };
    var s = svgOpen(W, H) + '<text class="tt" x="' + Lp + '" y="14">' + esc(title || m.name) + '</text>';
  ...
  function buildMeta() {
    var D = S.D, n34 = D.watch.sst_nino34, tw = D.watch.t2_world;
    $('pmeta').innerHTML = '<span>updated ' + esc(D.stamp) + '</span>' +
      '<span' + (n34.days_stale > 10 ? ' class="bad"' : '') + '>daily to ' + esc(n34.last_date) + ' (' + n34.days_stale + ' d)</span>' +
      '<span>NOAA week ' + esc(D.noaa.date) + '</span>' +
      '<span>land+ocean to ' + esc(tw.last_date) + '</span>' +
      (D.iri && D.iri.issued ? '<span>IRI ' + esc(D.iri.issued) + '</span>' : '');
    var dots = $('dots'); dots.innerHTML = '';
    Object.keys(D.sources).forEach(function (k) {
      var v = D.sources[k], i = el('i', v.fresh ? '' : 'bad');
      i.setAttribute('data-src', JSON.stringify({ name: v.label, def: v.fresh ? 'Source is fresh and was read on this update.' : 'Source did not answer: ' + (v.error || ''), src: k, date: D.stamp }));
      dots.appendChild(i);
    });
  }
  ...
  // ---------------------------------------------------------------- rails
  function railState() {
    var D = S.D, N = D.nino34, NW = D.noaa, ONI = D.oni, sm = D.summary || {};
    var col = $('railL'); col.innerHTML = '';
    var idx = D.risk_index, gc = idx >= 80 ? 'var(--lv5)' : (idx >= 60 ? 'var(--lv4)' : (idx >= 40 ? 'var(--lv3)' : 'var(--ok)'));
    var t = tile('State', esc(NW.type));
    var lat = NW.latest, ls = ONI.last_season;
    t._b.innerHTML = '<div class="gauge-row"><div class="gauge" data-term="riskindex" style="--v:' + idx + ';--c:' + gc + '"><div class="gv">' + idx + '</div><div class="gl">risk index<br>out of 100</div></div>' +
      '<div class="g-side"><b>' + term('nino34', 'Niño 3.4') + ' ' + fnum(lat.n34a, 1) + ' °C</b>' +
      'rank ' + N.all_years_rank + ' among all years for the same 30 days. ' + term('oni', 'ONI') + ' ' + fnum(ONI.current[ls]) + ' (' + esc(ls) + '), “very strong” starts at +2.0.</div></div>';
    var alerts = D.alerts || [];
    var a = el('div', 'alert' + (D.shout ? ' on' : (alerts.length ? '' : ' quiet')));
    if (alerts.length) {
      a.innerHTML = '<div class="ah">' + (D.shout ? 'ALERT: something happened that was never in the data' : 'Watchdog: shifts that need attention') + '</div>' +
        alerts.map(function (x) { return '<div class="arow ' + (x.level === 'SHOUT' ? 'lv-shout' : 'lv-watch') + '"><b>' + esc(x.level) + '</b>' + esc(x.title) + '<span class="d">' + esc(x.detail) + '</span></div>'; }).join('') +
        '<div class="src" style="margin-top:5px">' + term('shout', 'how the watchdog works') + '</div>';
    } else {
      a.innerHTML = '<div class="ah">The watchdog sees no turning point</div><div class="src">No rule fired: no record broken, no reversal, no run of records ended.</div>';
    }
    t._b.appendChild(a);
    var v = el('div', 'verdict');
    var tp = sm.turning_point || {}, cav = Array.isArray(sm.caveats) ? sm.caveats : (sm.caveats ? [sm.caveats] : []);
    v.innerHTML = '<div class="src">' + (sm.error ? T.aiRules + ' · ' + T.noModel : T.aiBy + ' · ' + esc(sm.model || '')) + ' ' + term('summary', '?') + '</div>' +
  ...
  function railRisks() {
    ...
    var risks = D.risks || [];
    var t = tile('Risks', risks.length + ' · index ' + D.risk_index, 'grow');
    ...
    risks.forEach(function (r, i) {
      ...
      c.innerHTML = '<div class="rl" style="background:' + lvlColor(r.level) + '">' + r.level + '</div>' +
        '<div><div class="rt">' + esc(r.title) + '</div><div class="rh">' + esc(r.horizon) + (r.metric ? ' · ' + esc(r.metric.name) : '') + '</div>' +
        (r.metric ? '<div style="margin-top:4px">' + spark(r.metric, 200, 26) + '</div>' : '') + '</div>';
      c.onclick = function () { S.risk = (S.risk === i ? null : i); S.view = S.risk == null ? S.view : 'risk'; render(); };
      box.appendChild(c);
    });
  ...
  // — вкладка «где мы»
  function viewNow() {
    var D = S.D, N = D.nino34, NW = D.noaa, ONI = D.oni, n34 = D.watch.sst_nino34;
    ...
    var body = stageShell(above ? 'Warmer today than any of the four strongest events were at this time of year'
      : 'The event follows the strongest ones: rank ' + N.rank_same30 + ' among the analogues',
      [segBtn('now', 'analogs', 'Against analogues', 'analogs'), segBtn('now', 'map', 'Pacific map', 'analogs'), segBtn('now', 'weekly', 'Weekly indices', 'analogs')]);
    if (k === 'map') plot(body, function (w, h) { return pacific(NW, w, h); });
    ...
    var cap = el('div', 'cap');
    cap.innerHTML = k === 'map' ? 'The four patches by which El Niño is judged; the number is the anomaly for the week of ' + esc(NW.date) + '. Point at a patch for its definition.'
      : (k === 'weekly' ? 'Four Niño regions weekly. Over 4 weeks: Niño 3.4 ' + fnum(c4.n34a, 1) + ', Niño 1+2 ' + fnum(c4.n12a, 1) + '; over 8 weeks ' + fnum(c8.n34a, 1) + ' and ' + fnum(c8.n12a, 1) + '. Record of the weekly Niño 3.4: ' + fnum(NW.hist_max_n34.n34a, 1) + ' (' + esc(NW.hist_max_n34.date) + ').'
        : '<strong>Peak estimate.</strong> ' + esc(pe.note) + ' Adding the analogues’ gain would give ' + fnum(pe.additive_low, 1) + ' … ' + fnum(pe.additive_high, 1) + ' °C; typical peak window ' + esc(pe.typical_peak_window) + '.');
    body.appendChild(cap);
    var kp = el('div', 'kpis');
    kp.innerHTML =
      '<div class="kpi"><div class="kn">' + term('weekly', 'NOAA weekly') + '</div><div class="kv">' + fnum(NW.latest.n34a, 1) + '<small>°C</small></div><div class="km">' + term('percentile', ord(Math.round(NW.n34_rank_pct)) + ' percentile') + ' of this season’s weeks</div><div class="kd"><span>4 w <span class="' + upDown(c4.n34a) + '">' + fnum(c4.n34a, 1) + '</span></span><span>8 w <span class="' + upDown(c8.n34a) + '">' + fnum(c8.n34a, 1) + '</span></span></div></div>' +
      '<div class="kpi"><div class="kn">' + term('oisst', 'daily OISST') + '</div><div class="kv">' + fnum(N.current_day) + '<small>°C</small></div><div class="km">30 days ' + fnum(N.current30) + ', ' + term('rank', 'rank ' + N.all_years_rank) + ' of all years</div><div class="kd"><span>slope ' + fnum(n34.slope14.now) + '</span><span>' + term('cusum', 'CUSUM') + ' ' + (n34.cusum.alarm ? 'alarm' : 'quiet') + '</span></div></div>' +
      '<div class="kpi"><div class="kn">' + term('oni', 'ONI official') + '</div><div class="kv">' + fnum(ONI.current[ls]) + '<small>' + esc(ls) + '</small></div><div class="km">analogues: ' + [1982, 1997, 2015, 2023].map(function (y) { return y + ' ' + fnum((ONI.analogs[y] || {})[ls]); }).join(', ') + '</div></div>' +
      '<div class="kpi"><div class="kn">' + term('type', 'event type') + '</div><div class="kv" style="font-size:15px;line-height:1.25">' + esc(NW.type) + '</div><div class="km">' + term('nino12', '1+2') + ' ' + fnum(NW.latest.n12a, 1) + ' · ' + term('nino4', '4') + ' ' + fnum(NW.latest.n4a, 1) + ' · east−centre ' + fnum(NW.east_minus_central, 1) + '</div></div>';
    body.appendChild(kp);
  }

  // — вкладка «модели»
  function viewModels() {
    var D = S.D, IRI = D.iri && !D.iri.error ? D.iri : null, NW = D.noaa;
    if (!IRI) { var b0 = stageShell('The IRI plume did not load', []); b0.appendChild(el('div', 'note warn', esc((D.iri || {}).error || ''))); return; }
    var ao = IRI.against_observed || {}, rv = IRI.revisions || {}, models = IRI.models, classes = IRI.classes || {};
    ...
    var body = stageShell(title, [segBtn('models', 'plume', 'Plume', 'plume'), segBtn('models', 'board', 'Scoreboard', 'plume'), segBtn('models', 'revision', 'Revisions', 'plume')]);
    ...
    if (k === 'plume') {
      ...
      body.appendChild(el('div', 'cap', 'Each thin line is one model (' + IRI.n_models + '; dynamical blue, statistical green); thick is the combined mean, dashed the previous issue. The red dot is this week’s reality on the first forecast season. Combined peak by issue: ' + hist.reverse().join(' → ') + '.'));
    } else if (k === 'revision') {
      var rows = (rv.rows || []).slice(0, 40);
      plot(body, function (w, h) {
        var W = w, Hh = h, Lp = 46, R = 20, Tp = topPad(W), B = 26, pw = W - Lp - R, ph = Hh - Tp - B;
        var vals = []; rows.forEach(function (r) { if (fin(r.peak_prev)) vals.push(r.peak_prev); if (fin(r.peak_cur)) vals.push(r.peak_cur); });
        ...
        var X = function (i) { return Lp + (rows.length < 2 ? pw / 2 : i / (rows.length - 1) * pw); };
        var Y = function (v) { return Tp + (vmax - v) / (vmax - vmin) * ph; };
        var s = svgOpen(W, Hh) + '<text class="tt" x="' + Lp + '" y="14">Peak revision by model, ' + esc(rv.prev_issued || '') + ' → ' + esc(IRI.issued) + ' (°C)</text>';
      ...
      body.appendChild(el('div', 'cap', (rv.n ? rv.n_up + ' of ' + rv.n + ' models raised their peak, ' + rv.n_down + ' lowered it; the combined peak went ' + fnum(rv.combined_peak_prev) + ' → ' + fnum(rv.combined_peak_cur) + ' °C. ' : '') + 'When almost all models move the same way, the models are catching up with the event rather than predicting it.'));
    } else {
      ...
      var list = names.map(function (nm) {
        ...
        return { name: nm, sec: m.section, v: v, gap: fin(v) ? v - ao.observed_weekly : null, peak: pk, dpk: r.d_peak, cls: c };
      ...
      wrap.innerHTML = '<table class="e"><thead><tr><th>model</th><th>type</th>' + (tally ? '<th>class</th>' : '') + '<th>' + esc(ao.season) + '</th><th>vs reality</th><th></th><th>peak</th><th>shift</th></tr></thead><tbody>' +
        list.map(function (r) {
          ...
          return '<tr data-model="' + esc(r.name) + '"><td>' + esc(r.name) + '</td><td>' + (r.sec === 'dyn' ? 'dyn.' : 'stat.') + '</td>' + (tally ? '<td>' + cc + '</td>' : '') +
            '<td class="num">' + fnum(r.v) + '</td><td class="num' + (r.gap != null && r.gap < 0 ? ' top' : '') + '">' + fnum(r.gap) + '</td><td>' + miniBar(r.v, ao.observed_weekly) + '</td><td class="num">' + fnum(r.peak) + '</td><td class="num' + ((r.dpk || 0) > .3 ? ' top' : '') + '">' + fnum(r.dpk) + '</td></tr>';
        }).join('') + '</tbody></table>';
      ...
      body.appendChild(el('div', 'cap', 'Click a row to light that model in the plume. The forecast for ' + esc(ao.season) + ' is a three-month mean while reality is a weekly point, so the comparison is honest only as “the model is below a level already reached”. Peak shift is against the ' + esc(rv.prev_issued || '—') + ' issue.'));
    }
    var tl = el('div', 'tally');
    tl.innerHTML = '<span><i style="background:var(--nino)"></i>below reality ' + ao.below.length + '</span><span><i style="background:var(--ok)"></i>above ' + ao.above.length + '</span>' +
      ...
      '<span>model mean ' + fnum(ao.mean) + ' ± ' + fnum(ao.sd, 2, false) + '</span>';
    body.appendChild(tl);
  }
  ...
  function viewTrend() {
    ...
    if (k === 'index') {
      ...
      body.appendChild(el('div', 'cap', 'Now ' + fnum(w0.last_value) + ' °C (to ' + esc(w0.last_date) + '); 30 days ' + fnum(w0.level30.anom) + ', ' + term('rank', 'rank ' + w0.level30.rank_raw + ' of ' + w0.level30.of) + ', ' + term('trend', 'above trend') + ' ' + fnum(w0.level30.det) + ' (z = ' + w0.level30.z + '); ' + term('analog', '+14 days') + ' ' + fnum(w0.forecast14.p10) + ' … <b>' + fnum(w0.forecast14.p50) + '</b> … ' + fnum(w0.forecast14.p90) + ' (' + term('p10p50p90', 'p10/p50/p90') + '); ' + w0.records.last30 + ' record days of 30, run ' + w0.records.streak + '; trend ' + fnum(w0.trend_per_decade) + ' °C per decade.'));
    }
  }

  // — вкладка «регионы и еда»
  function viewFood() {
    var D = S.D, RG = D.regions && !D.regions.error ? D.regions : null, FO = D.food && !D.food.error ? D.food : null;
    ...
    if (k === 'prices' && FO) {
      ...
      var G2 = FO.groups;
      body.appendChild(el('div', 'cap', 'Index ' + fnum(FO.index, 1, false) + ' in ' + esc(FO.last_month) + ': month ' + fnum(FO.mom, 1) + ', year ' + fnum(FO.yoy_pct, 1) + ' %. Year on year: ' + Object.keys(G2).map(function (g) { return g.toLowerCase() + ' ' + fnum(G2[g].yoy_pct, 1) + ' %'; }).join(', ') + '. The live food series lags by about a month.'));
      ...
      lead.innerHTML = 'Scenario: <b>' + scen + '</b>. base = combined model forecast (peak ' + fnum(RG.peak_p50) + ' °C), strong = top of the model spread (' + fnum(RG.peak_max) + '), record = reality above every model. Level = ' + term('teleconnection', 'typical impact') + ' × food ' + term('importer', 'vulnerability') + ' + scenario.';
      ...
      wrap.innerHTML = '<table class="e regions"><thead><tr><th>region</th>' + RG.seasons.map(function (s2) { return '<th>' + s2 + '</th>'; }).join('') +
        '<th>food</th>' + ['base', 'strong', 'record'].map(function (c) { return '<th data-scen="' + c + '"' + (c === scen ? ' class="sorted"' : '') + '>' + c + '</th>'; }).join('') + '<th>what to do</th></tr></thead><tbody>' +
        items.map(function (r) {
          var cells = RG.seasons.map(function (s2) {
            var x = r.seasons[s2] || {}, im = IMP[x.impact] || IMP.none;
            return '<td>' + srcSpanImp(im, x, r, RG, s2) + '</td>';
          }).join('');
          ...
          var lv = ['base', 'strong', 'record'].map(function (c) { return '<td class="num"><span class="lvl' + (c === scen ? ' cur' : '') + '" style="background:' + lvlColor(r.levels[c]) + '">' + r.levels[c] + '</span></td>'; }).join('');
          return '<tr><td>' + srcSpan({ name: r.name, def: r.countries + '. Vulnerability ' + r.vulnerability.level + ' of 5: ' + r.vulnerability.note + (r.vulnerability.importers && r.vulnerability.importers.length ? ' Net importers: ' + r.vulnerability.importers.join(', ') + '.' : ''), src: r.sources.join(' · '), date: RG.as_of }, r.name) +
      ...
      body.appendChild(wrap);
      body.appendChild(el('div', 'cap', 'Impacts are typical for a strong eastern-type El Niño, from published NOAA CPC and IRI impact maps and FAO GIEWS alerts: “usually”, never “will”. Click a scenario column to switch it; point at a region or a season for the note and source.'));
    } else {
    ...
    if (FO && k === 'regions') {
      ...
      kp.innerHTML = '<div class="kpi"><div class="kn">' + term('fao', 'FAO food price index') + '</div><div class="kv">' + fnum(FO.index, 1, false) + '<small>' + esc(FO.last_month) + '</small></div><div class="km">month ' + fnum(FO.mom, 1) + ' · year ' + fnum(FO.yoy_pct, 1) + ' %</div></div>' +
        '<div class="kpi"><div class="kn">strongest rise, year on year</div><div class="kv" style="font-size:17px">' + esc(worst) + '<small>' + fnum(G3[worst].yoy_pct, 1) + ' %</small></div><div class="km">' + esc(worst) + ' index ' + fnum(G3[worst].last, 1, false) + '</div></div>' +
        '<div class="kpi"><div class="kn">scenario in force</div><div class="kv" style="font-size:17px">' + esc(RG ? RG.current_scenario : '—') + '</div><div class="km">chosen by the data: reality against the model spread</div></div>';
      body.appendChild(kp);
    }
  }
  function srcSpanImp(im, x, r, RG, s2) {
    var pay = { name: r.name + ' · ' + s2, def: x.note || 'No consistent signal for this season.', src: r.sources.join(' · '), date: RG.as_of };
    return '<span class="imp" data-src="' + esc(JSON.stringify(pay)) + '" style="--c:' + im[1] + '">' + im[0] + (x.impact && x.impact !== 'none' ? ' <small>' + esc(x.strength || '') + '</small>' : '') + '</span>';
  }
  ...
  function viewHow() {
    ...
    if (k === 'glossary') {
      ...
      body.appendChild(t);
      body.appendChild(el('div', 'cap', 'climatereanalyzer.org (ERA5 2 m; OISST v2.1) · NOAA CPC weekly Niño indices and ONI · NOAA PSL ERSST v6 monthly Niño 3.4 · IRI/CCSR model plume · FAO Food Price Index. Daily OISST lags one to three weeks: the NOAA weekly data is always fresher, and where they disagree the panel trusts the weekly. The raw data of every update is stored verbatim with its date.'));
    } else if (k === 'changed') {
      body.appendChild(el('div', 'lead', (D.diff || []).map(function (d) { return '· ' + esc(d); }).join('<br>') || 'First update: nothing to compare with yet.'));
    } else {
      var m = el('div', 'note');
      m.innerHTML = '<strong>Method.</strong> Everything is computed on ' + term('anomaly', 'anomalies to 1991–2020') + ' taken from the source files themselves. “Rank” is the position of the same 30 calendar days among all years of the series. “Above trend” is after subtracting the linear warming; z is in units of the spread of the same windows across years. Slope and noise are compared with the same 14- and 30-day windows of the same season, so a percentile means “unusual for this time of year”. The 14-day forecast is ' + term('analog', 'analogue-based') + '. ' + term('cusum', 'CUSUM') + ' accumulates the deviation from the level at the start of the window in units of the series’ spread; threshold 5. The ' + term('riskindex', 'risk index') + ' is a saturating sum of levels: 100·(1 − exp(−Σ level^1.5/25)).';
      body.appendChild(m);
      var c = el('div', 'note warn');
      c.innerHTML = '<strong>Caveats.</strong> The climatereanalyzer series are global means; the focus of the event is visible only through the NOAA Niño regions. Daily OISST lags, and the watchdog is blind on it for that long. The ' + term('plume', 'IRI plume') + ' is extracted from a figure: a change of layout would break the parser, which must then fail loudly. The ' + term('summary', 'model summary') + ' is an interpretation, not a source: every number in it must be in the digest of facts. ' + term('teleconnection', 'Teleconnections') + ' are typical, not guaranteed.';
      body.appendChild(c);
    }
  }

  // — сцена одного риска (клик в правом борту)
  function viewRisk() {
    var D = S.D, r = (D.risks || [])[S.risk];
    if (!r) { S.view = 'now'; return viewNow(); }
    var body = stageShell(esc(r.title), [{ label: '← back to ' + T.tabs.now, on: false, click: function () { S.risk = null; S.view = 'now'; render(); } }]);
    if (r.metric) plot(body, function (w, h) { return chartMetric(r.metric, w, h, r.metric.name); });
    var d = el('div', 'lead');
    d.innerHTML = '<b>Level ' + r.level + ' · ' + esc(r.horizon) + '.</b> ' + esc(r.plain || '');
    body.appendChild(d);
  ...
  // ---------------------------------------------------------------- dock
  function initDock() {
    var dock = $('dock'), txt = $('dtxt');
    function show(target) {
      var k = target.getAttribute('data-term'), p = null;
      if (k && S.G[k]) { var g = S.G[k]; p = { name: g.name, def: g.def, why: g.why, src: g.src }; }
      else if (target.getAttribute('data-src')) { try { p = JSON.parse(target.getAttribute('data-src')); } catch (e) { p = null; } }
      if (!p) return;
      txt.innerHTML = '<b>' + esc(p.name || '') + '</b>' + esc(p.def || '') + (p.why ? ' ' + esc(p.why) : '') +
        (p.src || p.date ? '<span class="s">' + esc(p.src || '') + (p.date ? ' · ' + esc(p.date) : '') + '</span>' : '');
    }
    function clear() { if (!S.pinned) txt.innerHTML = '<span class="hint">' + T.dockHint + '</span>'; }
    function find(e) { return e.target.closest && e.target.closest('[data-term],[data-src]'); }
    document.addEventListener('mouseover', function (e) { var x = find(e); if (x && !S.pinned) show(x); });
    document.addEventListener('mouseout', function (e) { if (find(e) && !S.pinned) clear(); });
    document.addEventListener('click', function (e) {
      var x = find(e);
      if (x) { S.pinned = (S.pinned === x ? null : x); if (S.pinned) show(x); else clear(); return; }
      if (S.pinned && !e.target.closest('#dock')) { S.pinned = null; clear(); }
    });
    document.addEventListener('keydown', function (e) { if (e.key === 'Escape') { S.pinned = null; clear(); } });
    clear();
  }
  ...
  Promise.all([get('/data/enso/latest.json'), get('/data/enso/glossary.json').catch(function () { return {}; }), get('/data/enso/history.json').catch(function () { return []; })])
    .then(function (r) {
      S.D = r[0]; S.G = (r[1] && r[1].en) || {}; S.H = r[2] || [];
