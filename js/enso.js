...
(function () {
  ...
  function modelSpan(nm, text) {
    var pay = modelPay(nm), c = ((S.D.iri || {}).classes || {})[nm] || {};
  ...
  function sub(view, def) {
    var v = S.sub[view] || def;
    /* Кнопке «i» на графике нужен адрес «вид/подвид», а plot() про подвид не знает: у каждой
       сцены своё умолчание, и живёт оно в её собственном вызове sub(). Запоминаем здесь. */
    if (view === S.view) S._subKey = v;
    return v;
  }
  ...
  function chartMosaic(stack, obs, W, H) {
    ...
    /* ГДЕ МЫ (владелец 09.09: «непонятно, где мы мигаем и где наш интервал»). Прожитые сезоны
       из плюма: целиком прожитый — зелёная точка, начатые — отрезок «от прожитого до того, где
       может кончиться среднее» с точкой; самый прожитый из начатых мигает, как на плюме. */
    var POS = (IR.position || []).filter(function (p) { return seas.indexOf(p.season) >= 0; });
    var LF = IR.last_full_season && seas.indexOf(IR.last_full_season.season) >= 0 ? IR.last_full_season : null;
    ...
    names.forEach(function (nm, k) {
      ...
      if (LF) s2 += '<circle cx="' + X(seas.indexOf(LF.season)).toFixed(1) + '" cy="' + Y(LF.value).toFixed(1) + '" r="3.2" style="fill:var(--ok)"/>';
      POS.forEach(function (p) {
        var xp = X(seas.indexOf(p.season));
        if (fin(p.lo) && fin(p.hi)) {
          s2 += '<line x1="' + xp.toFixed(1) + '" y1="' + Y(Math.max(p.hi, p.todate)).toFixed(1) + '" x2="' + xp.toFixed(1) + '" y2="' + Y(Math.min(p.lo, p.todate)).toFixed(1) + '" style="stroke:var(--nino)" stroke-width="3" opacity=".6"/>';
          [p.lo, p.hi].forEach(function (v) { s2 += '<line x1="' + (xp - 3).toFixed(1) + '" y1="' + Y(v).toFixed(1) + '" x2="' + (xp + 3).toFixed(1) + '" y2="' + Y(v).toFixed(1) + '" style="stroke:var(--nino)" stroke-width="1.4" opacity=".9"/>'; });
        }
  ...
  var SCENE_ANCHORS = { now: ['block:type', 'block:peak'], risk: ['block:type', 'block:peak'], models: ['block:models'], food: ['block:food'], radiance: ['block:radiance'],
    'trend/rain': ['block:rain'], 'trend/spectral': ['block:spectral'], 'trend/mountains': ['block:landbox'], regions: ['block:landbox'], gulf: ['block:landbox'], air: ['block:peak'] };
    ...
    host.innerHTML = '<span class="ks-h" data-src="' + esc(JSON.stringify({ name: 'Main indicators', def: 'The value of the last reading and its change against the previous one, from the panel journal; the ones that moved come first. Click any to see its history.' })) + '">KPI</span>' +
      show.map(function (x) {
        ...
        return '<button type="button" class="ks' + (x.rec ? ' rec' : '') + '" data-go="' + esc(KPI_SCENE[x.k] || 'overview') + '"' +
          ...
          '<span class="ks-n">' + esc(STRIP_NAME[x.k] || x.r.title) + (KPI_SCENE[x.k] ? plainBtn(String(KPI_SCENE[x.k]).split('/')[0], String(KPI_SCENE[x.k]).split('/')[1] || '', 'ks-i', true) : '') + '</span></button>';
      }).join('');
    stripArrows(host);
  }
  ...
  /* МИНИ-ДАШБОРД ОТВЕТА. Владелец 09.09: «в ответе видеть наши KPI и графики, чтобы собрался
     из наших готовых визуалов мини-дашбордик с описанием и ответом». Модель ничего не рисует:
     из найденных единиц берутся ключи, а плитки, искры и карточки статистики — те же, что на
     брифинге и сценах. В ход сохраняется РЕЦЕПТ (ключи), не картинка: при открытии сохранённого
     исследования числа подставляются живые, на день просмотра. Порядок — сначала процитированные
     моделью, потом по сходству; на ход не больше 4 плиток, 3 искр, 2 карточек статистики. */
  ...
  function rsDashTrim(d) {
    d.kpis = d.kpis.slice(0, 4); d.risks = d.risks.slice(0, 3); d.stats = d.stats.slice(0, 2); d.scenes = d.scenes.slice(0, 3);
  ...
  function rsStatCard(id) {
    ...
    return '<a class="rs-dst" href="#' + esc(it.scene || 'overview') + '"><span class="rs-dst-t">' + esc(it.title) + '</span>' +
      '<span class="rs-dst-k">' + ks.map(function (k) { return '<span><b>' + esc(String(k.value)) + (k.unit ? ' ' + esc(k.unit) : '') + '</b> ' + esc(k.name) + '</span>'; }).join('') + '</span>' +
      '<span class="rs-dst-m">' + esc(rsFirstSentences(m.plain || '', 1)) + '</span></a>';
  }
  function rsDashHtml(dash) {
    ...
    return '<div class="rs-dash">' +
      (kp.length ? '<div class="rs-dk">' + kp.map(rsKpiTile).join('') + '</div>' : '') +
      (rk.length ? '<div class="rs-ds">' + rk.map(function (id) { return briefSpark(id, id, 170); }).join('') + '</div>' : '') +
      (st.length ? '<div class="rs-dt">' + st.map(rsStatCard).join('') + '</div>' : '') +
  ...
  function rsAsk(q) {
    ...
    rsPost({ question: q, lang: rsLang(q), history: history }).then(function (d) {
      ...
      if (d.error) {
        ...
        rsMerge(rs, hits); msg.a += rsDemoAnswer(q, hits, 'Meanwhile, what the panel itself says:'); msg.dash = rsDashFromHits(hits); render(); return;
      }
  ...
  function rsExportText(rs) {
    return 'Research on the El Niño panel · ' + rs.created + '\n\n' + rs.msgs.map(function (m, i) { return (i + 1) + '. Q: ' + m.q + '\n   A: ' + String(m.a || '').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim() + (m.dash ? '\n   Shown: ' + [].concat((m.dash.kpis || []).map(function (k) { var r = jrec(k); return r ? r.title : k; }), (m.dash.risks || []).map(function (r) { var x = briefRisk(r); return x ? x.title : r; }), (m.dash.stats || []).map(function (id) { var x = rsStat(id); return x ? x.title : id; })).join('; ') : ''); }).join('\n\n') +
  ...
  /* ЛЁД: тринадцать горных точек (владелец 14.09 — «города не только в зоне, горные районы,
     угрозы ледникам»). Показываем то, что меряется: аномалию воздуха тем же кирпичом, что
     Niño 3.4, и давление на лёд суммой градусо-дней. Счёт ДНЕЙ таяния сам по себе врёт:
     на высоте 2769 м каждый июльский день и так выше нуля, и рекордно жаркий год выглядит
     обычным (Эцталь: 145 дней при норме 145,5 — и при этом 814 градусо-дней против 536). */
  function iceRows() {
    var G = (S.IC || {}).series || {};
    return Object.keys(G).map(function (k) { return Object.assign({ key: k }, G[k]); });
  }

  function icePdd(r) { return (r.melt || {}).pdd || {}; }
  ...
  function viewGlaciers(body) {
    ...
    if (planned && rows.length < planned) {
      body.appendChild(el('div', 'note warn', esc(rows.length + ' of ' + planned + ' points on this run: ' +
        (errN ? 'the archive did not answer for the rest (rate limit). ' : '') +
        'The missing points are not missing ice, they are missing data.')));
    }
    var pick = S.sub.mtn || 'all';
    var sorted = rows.slice().sort(function (a, b) { return (icePdd(a).rank || 99) - (icePdd(b).rank || 99); });
    var row = el('div', 'seg sub');
    /* segBtn отдаёт описание для stageShell, а не узел: во втором ярусе кнопки строим сами,
       как на сценах цен и воды. */
    [['all', 'All points']].concat(sorted.map(function (r) { return [r.key, (r.label || r.key).split(',')[0]]; }))
      .forEach(function (o) {
        var b = el('button', (pick === o[0] ? 'on' : '') + ' sq', o[1]); b.type = 'button';
        b.onclick = function () { S.sub.mtn = o[0]; mScreen('trend/mountains/' + o[0]); render(); };
        row.appendChild(b);
      });
    body.appendChild(row);
    ...
    if (pick === 'all') {
      var wrap = el('div'); wrap.style.cssText = 'flex:1;min-height:0;overflow:auto';
      wrap.innerHTML = '<table class="e"><thead><tr><th>point</th><th class="num">grid, m</th>' +
        '<th class="num">air, 30 d</th><th class="num">rank</th><th class="num">melt energy</th>' +
        '<th class="num">normal</th><th class="num">rank</th><th class="num">per decade</th>' +
        '<th class="num">days above 0</th></tr></thead><tbody>' +
        sorted.map(function (r) {
          var pd = icePdd(r), m = r.melt || {};
          var hot = pd.rank === 1 ? ' top' : '';
          return '<tr><td style="white-space:nowrap;min-width:200px">' + esc((r.label || r.key).split(',')[0]) +
            '<div class="sub">' + esc(r.region || '') + '</div></td>' +
            '<td class="num">' + (r.elevation != null ? Math.round(r.elevation) : '·') + '</td>' +
            '<td class="num">' + fnum((r.level30 || {}).anom) + '</td>' +
      ...
      body.appendChild(wrap);
      body.appendChild(el('div', 'cap', 'Melt energy is the sum of daily mean temperatures above freezing since 1 January, in °C·day — the standard melt index of glaciology, and the honest one: counting days above zero saturates in summer and hides a hot year. “Normal” is the average of 1991–2020 to this same day of the year, so the running year is compared like with like. ' + esc((S.IC || {}).note || '')));
      worksFoot(body, 'block:landbox');
      return;
    }

    var r0 = rows.filter(function (r) { return r.key === pick; })[0];
    if (!r0) { body.appendChild(el('div', 'note', 'No such point.')); return; }
    plot(body, function (w, h) { return chartRecent(r0, w, h); });
    var pd = icePdd(r0), m0 = r0.melt || {}, sn0 = r0.snow || null;
    var kp = el('div', 'kpis');
    kp.innerHTML = '<div class="kpi"><div class="kn">melt energy this year</div><div class="kv">' + fnum(pd.to_date, 0, false) +
      ...
      '<div class="kpi"><div class="kn">air over the ice, 30 days</div><div class="kv">' + fnum((r0.level30 || {}).anom) +
        '<small>°C</small></div><div class="km">' + term('rank', 'rank ' + ((r0.level30 || {}).rank_raw || '·') + ' of ' + ((r0.level30 || {}).of || '·')) +
        '; last day ' + fnum(r0.last_value) + ' on ' + esc(r0.last_date) + '</div>' + kmeta(null, r0.source, r0.last_date) + '</div>' +
      '<div class="kpi"><div class="kn">days above freezing</div><div class="kv">' + (m0.to_date != null ? m0.to_date : '·') +
        '<small>of ' + fnum(m0.clim_to_date, 0, false) + ' normal</small></div><div class="km">the count saturates in summer; the energy above is the sharper number</div>' +
        kmeta(null, r0.source, r0.last_date) + '</div>' +
      (sn0 && sn0.now != null ? '<div class="kpi"><div class="kn">snow share of wet days</div><div class="kv">' + sn0.now +
        '<small>%</small></div><div class="km">the 1991–2020 average is ' + (sn0.clim != null ? sn0.clim : '·') + ' %; rain does not feed a glacier</div>' +
        kmeta(null, r0.source, r0.last_date) + '</div>' : '');
    body.appendChild(kp);
    body.appendChild(el('div', 'cap', '<strong>' + esc((r0.label || '').split(',')[0]) + '.</strong> ' + esc(r0.what || '') +
      ' <strong>El Niño:</strong> ' + esc(r0.enso || '') +
      ' Grid point ' + (r0.point ? fnum(r0.point[0], 2, false) + ', ' + fnum(r0.point[1], 2, false) : '') +
      ' at ' + (r0.elevation != null ? Math.round(r0.elevation) + ' m' : 'unknown height') + '; ' + esc(r0.source || '') +
      '. ERA5-Land is a 9 km grid, not the ice surface: these are indices of the pressure on the ice, not a mass balance. ' +
      'The measured mass balance is the annual WGMS record ' + vLink('on the Long term tab', 'planet', 'glaciers') + '.'));
    worksFoot(body, 'block:landbox');
  }
  ...
  /* ДЕРЖИТСЯ ЛИ СВЯЗЬ. Сборщик радианса v7 (10.09) добавил блок regime: он не спрашивает
     ...
     наклона держится на одном уровне, связь та же; уход полосы — смена режима. Полоса это
     ±одна стандартная ошибка наклона, пунктир — наклон по всем годам сразу. */
  function chartRegimeSlope(o, W, H) {
    var rows = (o.rows || []).filter(function (r) { return fin(r.slope); });
    if (rows.length < 2) return svgOpen(W, H) + '<text x="20" y="40">not enough windows for a rolling slope</text></svg>';
    var Lp = 52, Rp = 12, Tp = topPad(W), B = 26, pw = W - Lp - Rp, ph = H - Tp - B;
    var lo = rows.map(function (r) { return r.slope - (r.se || 0); }), hi = rows.map(function (r) { return r.slope + (r.se || 0); });
    var vmin = Math.min.apply(null, lo), vmax = Math.max.apply(null, hi);
    if (fin(o.full)) { vmin = Math.min(vmin, o.full); vmax = Math.max(vmax, o.full); }
    var pad = (vmax - vmin) * .18 || .1; vmin -= pad; vmax += pad * 1.7;
    var x0 = rows[0].to, x1 = rows[rows.length - 1].to;
    var X = function (x) { return Lp + (x - x0) / Math.max(1, x1 - x0) * pw; }, Y = function (v) { return Tp + (vmax - v) / (vmax - vmin) * ph; };
    var s2 = svgOpen(W, H) + '<text class="tt" x="' + Lp + '" y="13">' + fitText(esc(o.title || ''), W - Lp - Rp, 11) + '</text>';
    s2 += gridY(vmin, vmax, niceStep(vmax - vmin, 5), Y, Lp, Rp, W, 2);
    var step = Math.max(1, Math.round((x1 - x0) / (W < 520 ? 4 : 8)));
    for (var x = Math.ceil(x0 / step) * step; x <= x1; x += step) s2 += '<text x="' + X(x).toFixed(0) + '" y="' + (H - 9) + '" text-anchor="middle">' + x + '</text>';
    // полоса ±SE: снизу вперёд, сверху назад
    var up = rows.map(function (r) { return X(r.to).toFixed(1) + ',' + Y(r.slope + (r.se || 0)).toFixed(1); });
    var dn = rows.map(function (r) { return X(r.to).toFixed(1) + ',' + Y(r.slope - (r.se || 0)).toFixed(1); }).reverse();
    s2 += '<polygon points="' + up.concat(dn).join(' ') + '" style="fill:var(--ochre)" opacity=".16"/>';
    if (fin(o.full)) {
      s2 += '<line x1="' + Lp + '" y1="' + Y(o.full).toFixed(1) + '" x2="' + (W - Rp) + '" y2="' + Y(o.full).toFixed(1) + '" style="stroke:var(--soft)" stroke-width=".9" stroke-dasharray="4 3"/>';
      s2 += '<text x="' + (W - Rp - 2) + '" y="' + (Y(o.full) - 4).toFixed(1) + '" text-anchor="end" opacity=".85">all years ' + fnum(o.full, 2, false) + '</text>';
    }
    s2 += segs(rows.map(function (r) { return [X(r.to), Y(r.slope)]; }), 'var(--ochre)', 2, 1);
    var last = rows[rows.length - 1];
    s2 += nowDot(X(last.to), Y(last.slope), 'var(--ochre)', 3.4);
    s2 += '<text x="' + (X(last.to) - 7).toFixed(1) + '" y="' + (Y(last.slope) - 10).toFixed(1) + '" text-anchor="end" font-size="12" style="fill:var(--text);paint-order:stroke;stroke:var(--surface);stroke-width:3;stroke-linejoin:round;font-weight:700">' + fnum(last.slope, 2, false) + '</text>';
    return s2 + '</svg>';
  }
  ...
  function viewRadiance() {
    ...
    var body = stageShell(head, [segBtn('radiance', 'convection', 'Convection', 'convection'), segBtn('radiance', 'walker', 'Raw Walker', 'convection'), segBtn('radiance', 'clouds', 'Cloud floors', 'convection'), segBtn('radiance', 'greenhouse', 'Window trap', 'convection'), segBtn('radiance', 'profile', 'Layers through cloud', 'convection'), segBtn('radiance', 'epochs', 'Twenty years', 'convection'), segBtn('radiance', 'regime', 'Is the link holding', 'convection'), segBtn('radiance', 'cross', 'Instruments', 'convection'), segBtn('radiance', 'seismic', 'Quakes and sun', 'convection')]);
    ...
    var srcHtml = '<b>Measured by us, from raw granules.</b> NOAA-21 CrIS (infrared, 2223 channels, the long-wave band read by HTTP range) and ATMS (microwave, 22 channels) straight from the anonymous NOAA NODD bucket; USGS earthquake catalogue; GFZ Potsdam Kp, sunspots, F10.7. Boxes Niño 3.4 and the warm pool, day (13:30) and night (01:30) nodes; window ' + esc(dl ? dl(0) : W0.start) + ' → ' + esc(W0.end) + ', years ' + (W0.years || []).join(', ') + '; updated ' + dt(String(RA.updated || '').slice(0, 10)) + '. The collector (C:\\CL\\radiance, schema v2) is run by its author; the panel copies the result when it is complete and re-derives the headline numbers from the layer-1 tables (radiance_check.py). Collector alerts: ' + (alertsEn.length ? alertsEn.join(' · ') : 'none') + '.' +
      (depthLine() ? ' Record depth of each sounder, from the collector’s own field rather than from the years kept in this file: ' + esc(depthLine()) + '.' : '');
    /* ГЛУБИНА ЗАПИСИ — ИЗ ФАЙЛА, А НЕ ИЗ ЧИСЛА ЛЕТ В РЯДАХ. Просьба автора сборщика к v7:
       у каждого прибора есть поле years_available, и подпись надо строить по нему. Ряды в
       панельном файле обрезаны окном (2023–2026), и по ним глубина AIRS выглядела бы как
       четыре года вместо двадцати четырёх. */
    function depthOf(inst) {
      var ya = ((SRC[inst] || {}).years_available) || {};
      return ya.from ? '; record ' + ya.from + '–' + ya.to + ', ' + ya.n_years + ' years' : '';
    }
    function depthLine() {
      // имена приборов свои: label в файле сборщика русский, а панель английская
      var NM = { n21_cris: 'NOAA-21 CrIS', n20_cris: 'NOAA-20 CrIS', snpp_cris: 'SNPP CrIS', aqua_airs: 'Aqua AIRS' };
      return ['n21_cris', 'n20_cris', 'snpp_cris', 'aqua_airs'].filter(function (q) { return (SRC[q] || {}).years_available; })
        .map(function (q) { var ya = SRC[q].years_available; return NM[q] + ' ' + ya.from + '–' + ya.to + ' (' + ya.n_years + ' years)'; }).join(' · ');
    }
    ...
    function platRow(row) {
      ...
      row.appendChild(el('span', 'seg-gap', ''));
      [['cris', 'NOAA-21 CrIS'], ['n20', 'NOAA-20 CrIS'], ['airs', 'Aqua AIRS']].filter(function (o) { return o[0] !== 'n20' || N20.series; }).forEach(function (o) {
        var b = el('button', (plat === o[0] ? 'on' : '') + ' sq', o[1]); b.type = 'button';
        b.title = (o[0] === 'airs' ? 'independent platform; its overpass has drifted to ~15:40 local' : (o[0] === 'n20' ? 'the same instrument on NOAA-20, overpass ~13:31' : 'NOAA-21, overpass 13:25 local')) + depthOf({ airs: 'aqua_airs', n20: 'n20_cris', cris: 'n21_cris' }[o[0]]);
        b.onclick = function () { S.sub.radPlat = o[0]; render(); }; row.appendChild(b);
      });
      return row;
    }
    ...
    if (k === 'convection') {
      ...
      /* СЛЕЖЕНИЕ ЗА СМЕНОЙ РЕЖИМА (сборщик v7, 10.09). Автор сборщика передал две оговорки,
         ...
         1. is_extrapolation истинно во всех восьми вариантах: температура этого года выше
            всего, на чём связь училась, и остаток года от связи — не замер, а утверждение о
            форме кривой за пределом данных. Значит остаток не выносим числом наверх.
         2. Остатки в текущей выдаче завышены по модулю в 1,3–1,8 раза из-за дефекта с
            подписью ошибки предсказания; автор чинит. Значит и историю остатков по годам,
            и σ-числа держим вне экрана до починки — показывать заведомо неверный масштаб
            хуже, чем не показывать.
         Остаётся то, на что дефект не влияет: сам наклон связи, его дрейф по скользящему
         окну лет, доля наблюдённой конвекции от предсказанной, и согласие вариантов по
         знаку. Этого хватает, чтобы ответить на вопрос сцены: связь держится или нет. */
      var RG = SRC.regime || {}, RGV = RG.variants || {};
      var vkeys = Object.keys(RGV).sort();
      if (!vkeys.length) { body.appendChild(el('div', 'note', 'No regime block in radiance.json yet: it appears in the collector from v7.')); return; }
      var rwin = S.sub.regWin || 'season', rpair = S.sub.regPair || 'aqua_airs|A';
      var pairs = [], seenP = {};
      vkeys.forEach(function (q) { var p = q.split('|').slice(0, 2).join('|'); if (!seenP[p]) { seenP[p] = 1; pairs.push(p); } });
      var wins = [];
      vkeys.forEach(function (q) { var w = q.split('|')[2]; if (wins.indexOf(w) < 0) wins.push(w); });
      if (pairs.indexOf(rpair) < 0) rpair = pairs[0];
      if (wins.indexOf(rwin) < 0) rwin = wins[0];
      var vkey = rpair + '|' + rwin, V = RGV[vkey] || {};
      var INAME2 = { n21_cris: 'NOAA-21 CrIS', n20_cris: 'NOAA-20 CrIS', snpp_cris: 'SNPP CrIS', aqua_airs: 'Aqua AIRS' };
      var WNAME = { season: 'the season', ytd: 'the year so far' };
      function pairName(p) { var a = p.split('|'); return (INAME2[a[0]] || a[0]) + ' · ' + (a[1] === 'A' ? 'day' : 'night'); }
      row = el('div', 'seg sub');
      pairs.forEach(function (p) { var b = el('button', (rpair === p ? 'on' : '') + ' sq', pairName(p)); b.type = 'button'; b.onclick = function () { S.sub.regPair = p; render(); }; row.appendChild(b); });
      row.appendChild(el('span', 'seg-gap', ''));
      wins.forEach(function (w) {
        var wd = (RG.windows || {})[w] || {};
        var b = el('button', (rwin === w ? 'on' : '') + ' sq', (WNAME[w] || w) + (wd.from ? ', ' + wd.from + ' to ' + wd.to : ''));
        b.type = 'button'; b.onclick = function () { S.sub.regWin = w; render(); }; row.appendChild(b);
      });
  ...
  var PLAIN = {
    ...
    "air/fuel": {"title": "The warm water stored below", "what": "How much warm water is piled up under the surface along the equator in the Pacific — the reserve the event feeds on.", "see": "The heavy dark line is that volume month by month over the last five years, with a dot on the latest month; the left scale, in units of a hundred trillion cubic metres, belongs to it alone. The dashed warm line is the surface temperature of the central Pacific pulled back by the number of months the volume leads it — a lead we find by trying every shift from zero to twelve months, not one taken from a textbook — so the two lie one over the other; it carries no scale of its own, so read its shape and not its height. Four cards follow: the volume as a share of the highest value since 1980; the month it peaked in and whether it has begun to be spent; the lead itself, with how closely the two move together; and the same heat written as a temperature of the upper 300 metres.", "special": "The surface tells you what is happening; this tells you what is still in store. It is the panel's slowest early sign, and the lead of several months is measured here against our own Niño 3.4 series rather than assumed.", "src": "NOAA PMEL/TAO warm water volume and upper-300 m temperature, monthly"},
    "air/indices": {"title": "Other people's indices, beside ours", "what": "Three measures of the same event built by other institutions — one that combines five different fields, one for the Indian Ocean, one that subtracts the general warming — shown next to our own three-sign score.", "see": "A pair of buttons picks which of the two to draw — the five-field index, whose latest value is a two-month mean, or the monthly Indian Ocean one. The heavy line is that index over the last five years, and straight dashed lines mark the same month of the strongest past events, each labelled with its year; under the chart, three cards: the five-field index with its value in the same month of those events, the Indian Ocean dipole with the phase it is in, and RONI with the gap between it and the official ONI.", "special": "Our coupling score is our own arithmetic; these are built by other people from other data, so a disagreement with us is visible on one screen. RONI is the one to read first: it removes the warm background of recent decades, which is what lets today's event be set against 1982 on the same footing.", "src": "NOAA PSL for the five-field index (two-month means) and the Indian Ocean dipole (monthly); NOAA CPC for RONI, by three-month season"},
    "air/layers": {"title": "How high the warmth climbs", "what": "Four heights in the atmosphere, from the air just above us up to the stratosphere, each measured by satellite as its distance from its own normal.", "see": "Four stacked panels share one temperature scale and one time axis along the bottom, three years of months. In each panel the coloured line is that layer month by month with a dot on its latest value, and the straight dashed lines mark where the same layer stood after the events of 1997, 2015 and 2023. The column to the right of each panel gives today's value and the delay between the ocean and that layer; on a narrow screen those labels move into the legend under the heading.", "special": "It answers something no sea-surface chart can: how far up the event has reached and how long each floor takes to answer. The top floor is the odd one — the note says the stratosphere cools during a strong El Niño while the floors below warm — and that is why the delay and the closeness of fit printed beside it read weak.", "src": "UAH satellite temperatures of four layers, monthly; the delays are measured by us against our own Niño 3.4 series"},
    "air/mjo": {"title": "The pulse that circles the tropics", "what": "A pulse of cloud and wind that travels around the tropics every 30 to 60 days, shown as where it is now and how strong it is.", "see": "The wheel is a map of where the pulse sits: eight numbered sectors around a circle, the western Pacific ones picked out in warm colour, with a trail of the last 30 days ending in a larger dot on the latest day the index carries. Inside the small dashed circle the pulse is too weak to count as organised. Beside the wheel — below it on a narrow screen — the daily strength of the pulse over the last three months, with the days that fall in the western Pacific sectors at full strength shaded.", "special": "This is the calendar for the wind bursts on the neighbouring chart: when the pulse crosses the western Pacific strongly, the window for a burst is open, and the third card says whether it is open now. That window is our own count of how many of the last 15 days met both conditions, not a published index.", "src": "NOAA PSL real-time OMI (ROMI), daily"},
    "air/wind": {"title": "Daily wind over the western Pacific", "what": "How far the daily east-west wind over the western equatorial Pacific sits from its normal for that day of the year, and the short spells when it turns westerly hard enough to push warm water eastward.", "see": "One bar per day for the last 120 days: warm bars above the line are days more westerly than normal, hatched cool bars below are days more easterly than normal, which is the usual direction there. A dashed line marks the threshold we use for a burst — twice the usual daily swing — the spells that qualify are shaded across the full height, and the latest value is labelled at the right end. The cards give the last week's average, list each burst with its dates, length and peak, and repeat the same measure at the moorings on the equator, each against its own record.", "special": "A burst of westerly wind is what launches the pulse of warm water that reaches the South American coast two to three months later, so this is the earliest step of the next move rather than a picture of the present one. The burst rule — two standard deviations for five days or more, averaged over six points — is ours, and the threshold is drawn on the chart so you can see it being applied.", "src": "ERA5 10 m wind at six equatorial points from 130°E to 180° via the Open-Meteo archive, daily, against our own 1991–2020 normal; mooring winds from TAO/TRITON"},
    "brief": {"title": "One page to start from", "what": "A written walk through the whole event, from what is happening now to what to watch next, with the panel's live numbers set into the sentences.", "see": "Seven sections in order: the event in one paragraph, what the data show at the surface, below it, in the air and in the models, what to expect and when, the risks that have already fired, regions and food, what to watch in the coming weeks, and how to read the panel. Beside the text sit small number tiles with their change, and small line charts of a single series with its latest value, which open the risk page that series belongs to. Under each block runs a row of short arrow links to the scenes where its numbers are drawn, so a claim can be followed back rather than taken on trust.", "special": "It is the one page that puts the separate readings in order and says what they add up to for someone arriving cold, and it names its own arithmetic where it appears: the likely winter peak and the odds of beating 1997 are worked out from past events, not taken from anyone's forecast. The sentences are written by hand and only the figures in them refresh, so the page prints the date of the data; the few lines that do come from the model, at the end of the watch list and in the closing caveats, say so.", "src": "hand-written text with live slots; the numbers are read at render time from the panel's own files"},
    "globe": {"title": "The planet with its layers on", "what": "One globe you can turn, with several measured fields laid over each other — each with its own switch and its own strength.", "see": "The sphere carries the sea surface underneath, coloured by how far it sits from normal for the date: red warmer, blue colder. Over it lies the cloud layer in white — the brighter the white, the taller and colder the storm tops standing there right now, so the white bands are where the rain is falling. The switches on the left turn each layer on and off; the slider beside a layer sets how strongly it shows through the ones below. Boxes and buoys sit on top of everything, and the globe turns by itself until you grab it.", "special": "Every other view on this panel cuts the planet into a line, a box or a section. This one keeps it whole, so you can see that the warm water, the storm clouds and the boxes we watch are all the same piece of ocean — and watch the cloud band sit east of where it normally would.", "src": "sea surface from the NOAA OISST daily grid; cloud from the NOAA outgoing-longwave climate record, daily 1°; boxes and buoys from the same sources as their own scenes"},
    "food/abs": {"title": "What a tonne actually costs", "what": "The price of each good in dollars over the last five years, with the month this event began marked on the line.", "see": "\"All together\" draws twelve small charts at once, each a bold line in dollars per tonne (sugar per kilogram), shaded from the start of the event onward and with thin lines for what that good did after the starts of 1982, 1997, 2015 and 2023. \"One bundle\" puts all twelve on one picture instead, each as a percentage of its own price in the starting month, so anything above the 100 line has risen since. Pick a single name and you get the full chart with the five-year high and low marked, a circle on the price a year ago, and number cards for the month, the year and where the past events stood twelve and twenty-four months in.", "special": "The other food tabs are index points and percentages; here the line is in dollars, which is what an importer actually pays. Pick a single good and the five-year high and low are drawn right across the chart, so today's price can be placed against them; the toggle above decides whether the past paths run through this event's starting price or through today's - the same paths answering two different questions.", "src": "World Bank Pink Sheet monthly prices, nominal dollars, no adjustment for inflation"},
    ...
    "planet/glaciers": {"title": "What the glaciers lose each year", "what": "How much water the world's measured glaciers give up or gain in a year, and how much they have given up in all.", "see": "Two buttons switch the picture. \"Cumulative loss\" is one falling line of the running total in metres of water thickness, added up from the first year that had enough measured glaciers to count - where the axis starts, not where the archive starts; \"year by year\" is one bar per year, warm below the zero line for a year the ice lost water and cool above it for a year it gained. The two cards above give the latest complete year - the middle value across the glaciers measured that year, in millimetres of water - and that running total.", "special": "It is the only record here measured on the glaciers themselves rather than from a satellite, and adding the years up turns a string of small annual numbers into the one figure a reader can hold: the thickness of water the average measured glacier no longer has.", "src": "WGMS Fluctuations of Glaciers, annual archive; years with fewer than 30 measured glaciers are left out as preliminary"},
    "planet/ice": {"title": "Sea ice at both poles", "what": "How much of the ocean around each pole is covered by ice, on every day of every year since satellites began watching.", "see": "Each year is one faint line running January to December; this year is the thick ochre one, the dashed grey line is the middle value for each date over 1981-2010, and the years the strongest El Ninos began have their own colours and dashes. A button switches between Arctic and Antarctic, and the card below gives the covered area in millions of square kilometres, how far that is from the middle value, its rank for the same date among all years, and the record low. Days before 1988 were measured every other day, and the gaps between them are filled in.", "special": "Stacking the years on one calendar instead of stringing them end to end shows the shape of a season, so you can tell a year that is low all the way through from one that lost its ice at a single point in the year.", "src": "NSIDC Sea Ice Index version 4, daily since October 1978"},
    "planet/sea": {"title": "Sea level, measured from orbit", "what": "How much higher the ocean stands than it did when satellites began measuring it in 1993.", "see": "A single line across the years, in millimetres above the level at the start of the record, with a dot and the number at the right end and warm dashed verticals at the years strong El Ninos began. The card below gives the total rise since 1993 and the speed of that rise in millimetres a year, both across the whole record and over the last ten years alone - the two rates side by side say whether it has quickened.", "special": "It carries the rise and the wobble in one picture: El Nino years bulge above the trend, because a warm Pacific keeps more water in the ocean and leaves less of it on land.", "src": "NOAA STAR satellite altimetry since 1993, with the seasonal cycle removed"},
    "planet/snow": {"title": "Snow cover of the north", "what": "How much land in the northern half of the world is under snow, averaged over each year of a record that begins in the 1960s.", "see": "Buttons choose the whole northern hemisphere, Eurasia, North America, or the weekly version of the hemisphere. The chart is one line of the yearly mean area in millions of square kilometres, with the newest year marked and labelled at the right end. The card above it takes the latest month or week on its own: its value, how far it sits from the average for that same period, and its rank among all the years of the record.", "special": "There is no open measurement of how much sunlight the planet throws back, and snow is the closest stand-in that exists in one file: it is the seasonal switch that makes land bright. It is measured as area, not depth, so a warm winter with the same cover can hold much less water.", "src": "Rutgers Global Snow Lab, monthly and weekly series since 1966"},
    "planet/temperature": {"title": "Temperature, every year as a line", "what": "How warm the world has been - daily readings for recent decades, yearly ones going back to 1850.", "see": "A row of buttons picks the series: land and ocean together, the ocean alone, land by month, either hemisphere, the tropics, either polar region, or the two long yearly records. The daily ones are drawn like the sea ice - each year a faint line across the calendar, this year thick in ochre, the average for the same date dashed with its years named in the legend - and they carry actual temperatures, while land by month and the two records since 1850 carry the difference from a long-run average instead. The strong El Nino years and the years right after them are picked out in colour on the year-as-a-line charts; the records since 1850 are bars instead, red above the average and blue with hatching below, with warm dashed verticals on those same years.", "special": "This is where the event and the long record meet: 1998, 2016 and 2024 each set the temperature record of their time, and each followed a strong El Nino by a few months, because the ocean hands its heat to the air late. Whether any of them still holds the record is answered by the card under the chart, which names the warmest year of the series.", "src": "ERA5 and OISST daily via climatereanalyzer, NOAA NCEI monthly for land, Met Office HadCRUT5 and CRUTEM5 for the yearly records"},
    "radiance/clouds": {"title": "Every scene sorted by cloud height", "what": "Each day's satellite scenes are sorted by how cold they read, and the chart follows the share that falls in whichever kind of sky you pick.", "see": "The buttons choose the bin: clear sky, meaning the satellite reads within four degrees of the sea itself, then low cloud, middle cloud and deep storm cloud, each bin set by its own temperature range. The chart is a line per year, this year thick and the three earlier years thin and dashed; a table button replaces it with all four bins at once, this year's last fourteen days in bold beside each earlier year's window average.", "special": "The convection view counts only the tallest cloud, so this is where the rest of the sky appears, thickening over one box while the other clears. Daytime cloud share is also the panel's stand-in for how much sunlight is turned back, and the notes say so plainly, because an infrared instrument cannot measure reflected light.", "src": "the same NOAA-21 infrared granules; the clear-sky bin also uses NOAA's daily sea-surface temperature"},
    "radiance/convection": {"title": "Storm clouds counted from orbit", "what": "A daily count of how much of the satellite's view over one Pacific box is filled with the very cold tops of tall storm clouds.", "see": "One line per year on the same calendar: this year thick, with a dot at its last day, and 2023, 2024 and 2025 thinner and dashed behind it. The left axis is the share of the day's satellite views cold enough to count as storm cloud, meaning colder than 235 kelvin, about 38 degrees below freezing. The tiles under the chart set this year's last fourteen days beside each earlier year's average over the whole window, which is the panel's own arithmetic, not a number from the file.", "special": "Sea temperature is the ocean's side of El Nino; this is the air's side, showing where the storms have actually gone. The box buttons switch between the middle of the Pacific and the warm western pool, so you can watch cloud leave one and arrive in the other.", "src": "raw NOAA-21 infrared granules from NOAA's open archive, measured for this panel by its own collector and refreshed daily"},
    "radiance/cross": {"title": "Two satellites checked against each other", "what": "A table asking whether separate satellites, passing over hours apart, recorded the same change this year.", "see": "Nine rows: the storm-cloud share over each box by day and by night, then five rows of the layers above them — the stratosphere over the central Pacific, the upper air over each box, and the surface-or-cloud-top window over each box. The columns are how far this year has moved from the 2023 to 2025 average as seen by Aqua AIRS, by NOAA-21 and, when the file carries it, by NOAA-20, and the last column is the gap between the first two, turning green when it is within one point of cloud share or half a degree. The tiles below give the day-by-day agreement between the instruments for storm cloud and for the east-west contrast.", "special": "The other views show one instrument at a time; this one sets them side by side, so a reader can see whether the finding lives in the sky or in one detector. It also shows where the instruments ought to differ: the surface window channel is degrees apart because the satellites pass at different hours and catch cloud at different points of its daily cycle.", "src": "computed inside the panel from the daily series of the three instruments in the collector's file, not copied from its own text"},
    "radiance/epochs": {"title": "The same weeks, every year since 2003", "what": "One bar for every year since 2003, each the average over the same stretch of the calendar, so a year from two decades ago can stand beside this one.", "see": "Bars run left to right by year and are coloured by the instrument that measured that year; the thin whisker on each shows how uncertain that year's average is, and the dashed line across the chart is the middle value of the years 2003 to 2021. This year's bar is at the right and pulses. A play button and a slider grow the record year by year, and the buttons above switch the box, the hour of the overpass, and the quantity between the share of storm cloud, the channel that reads the surface or the cloud top, and the channel that reads the air high above it.", "special": "Four instruments cover these years and no two read exactly alike, so each older one is shifted onto NOAA-21's scale using the days when both were flying at once. That shift is what makes an old bar comparable at all, and the tiles below give this year's rank among all the bars and its ratio to 2015.", "src": "the collector's epoch block: Aqua AIRS for the early years up to 2021, then the CrIS chain of SNPP, NOAA-20 and NOAA-21"},
    ...
    "regions/place": {"title": "One region at a time, measured", "what": "A single region chosen from a list, with whatever is actually measured on the ground there shown above the written reference for it.", "see": "The dropdown lists all seventeen regions with the level each carries under the current scenario, and marks the Gulf as the one measured here. The Gulf and Arabian Peninsula opens five tabs: sea temperature in the Gulf box off Kuwait, as distance from its normal or in plain degrees; Kuwait's daily maximum and its rain against normal; a winter outlook that is quoted literature and is labelled as such; the import chain with world wheat and rice prices; and the written reference. Five other regions have a land box of their own — Europe, Peru and the Andes, Southeast Asia and Indonesia, East Africa, South Asia — and open with a daily air chart, the band of all years behind it, and cards for the last day, the fourteen-day outlook, record days and rain against normal; a region with nothing measured opens straight into its reference: effects by season, food exposure, what to do, and the sources.", "special": "It is the one place where the general reference and the local measurement stand side by side, so you can see how much of what is said about a region is actually watched here and how much is quoted from elsewhere. Where nothing local exists yet, the page says so rather than filling the gap.", "src": "The same regional reference, plus our own OISST box, ERA5 series and World Bank monthly prices"},
    "regions/table": {"title": "All seventeen regions on one sheet", "what": "A hand-built reference of what an El Niño of this size usually does to each of seventeen regions, season by season, with how exposed each one is through its food.", "see": "One row per region, ordered by severity: three season columns — SON is September to November, DJF December to February, MAM March to May — each carrying a word for the usual effect (drought, heat, wet, floods, or no signal) and how strong it is, then five bars for how exposed the region is through food. Then three numbers, one per scenario (base, strong, record): a level of 1 to 5 that we work out ourselves, weighing how strongly the event usually shows in that region together with its food exposure and a bump for the scenario; and last the practical advice. The scenario buttons above move the highlight from one of those columns to another, the line over them says how many of the forecast models — the plume, the panel's word for the whole set of seasonal forecasts — reach each scenario's peak, which is a share of models and not a probability, and the cards at the foot carry the FAO food price index, the group rising fastest year on year, and the scenario the data currently put in force.", "special": "This is the only place where every region can be compared at once, and all three scenarios stand side by side in the same row, so you can see how much a region's level depends on how strong the event turns out. Every cell is 'usually', taken from published impact maps and the regional literature — never a statement that this will happen this winter.", "src": "A hand-written regional reference with its sources cited; the scenario shares come from the IRI set of seasonal model forecasts"},
    "strip": {"title": "The main numbers, and their change", "what": "The row of buttons under the menu holding the panel's main numbers, each with how far it has moved since it last moved.", "see": "Every button shows the latest value with its unit and the name of the indicator underneath; where the number has moved since the reading before it, an arrow and the size of the move stand beside it. Anything standing at a record is pushed to the front, outlined in red with a small \"record\" tag and a slow pulse; after the records come the numbers that moved, and the ones that stayed put are last. At most twelve are built out of a longer list of candidates: the round arrows at the edges appear only while there is more to scroll that way, and clicking a button opens the scene that number is drawn on.", "special": "The change here is not \"since the last refresh\" but \"since this number itself last changed\": the panel stores a reading only when the value or the date of its data moves, so a weekly or monthly figure shows a real step instead of a row of zeros. It sits under the menu rather than inside a scene, so the headline numbers, and any record among them, travel with you from chart to chart; a scene opened full screen is the one thing that covers it.", "src": "the panel's own value journal, which gains a reading only when a number or its data date changes"},
    "track/all": {"title": "Every forecast since 2002, raw", "what": "Every monthly forecast since 2002 on one picture - one line per issue, the average over that issue's models, with nothing averaged across the issues.", "see": "Each thin line is one monthly issue - the average over all the models of that issue - starting at the month it was published and reaching forward; its colour is the decade it came from, and the newest issue is the thick ochre one. The heavy line is the value that actually arrived. Where a bundle of thin lines sits above that line, the centres of that month were reading the coming season too warm; where it sits below, too cold.", "special": "Because the issues are not averaged with each other, the change across the archive appears as a shape - the older lines fan out far wider than the recent ones. The cards below also say how many issues are drawn and how many could not be parsed or were never published, rather than quietly dropping them.", "src": "Every IRI/CPC plume issue since 2002 that parsed, with NOAA's official values as the heavy line."},
    "track/error": {"title": "Forecast error, year by year", "what": "The average size of the centres' miss, year by year, for forecasts made a chosen distance ahead of the season they describe.", "see": "Four buttons pick how far ahead the forecast was made: one, three, six or nine seasons. The ochre line with dots is the average miss in degrees of the combined forecast, placed by the year the issue was published; the pale long-dashed line is the same miss for a forecast that just says neutral - no departure from normal - every season, scored on exactly the same targets. The gap between the two lines is what the centres add.", "special": "Raw error by year says nothing on its own - a quiet year is easy for free, and without that pale reference line a calm decade reads as skill. This is the only scene that carries the reference.", "src": "All parsed IRI/CPC plume issues since 2002 against NOAA's official values; both lines are our arithmetic."},
    "track/event": {"title": "One event, forecast by forecast", "what": "For one El Nino at a time, the peak the centres forecast at each monthly issue, against the peak that actually arrived.", "see": "Pick a year with the buttons; the event now running is marked. The ochre line with dots is the highest value the combined forecast gave anywhere inside that event's winter window, one dot per monthly issue, with issue names along the bottom where they fit, and the pale band around it is the range of the single models' own highest points in that same issue. The heavy flat line is what the official value reached - for an event still running there is no such line, and the cards say so rather than showing an error.", "special": "It shows the shape of a whole forecast season: whether the centres walked up to the event month after month or had it from the first issue. The peak is taken over one fixed window of seasons, which is what makes an early issue and a late one comparable at all.", "src": "Every IRI/CPC plume issue since 2002 that we could parse, against NOAA's official values."},
    "track/settle": {"title": "When the forecast stopped moving", "what": "How many months before the peak the centres' number settled down and stayed there.", "see": "One row per finished event, the year on the left and months before the peak running along the bottom. The open dashed bar reaches back to the first issue after which every later issue stayed within a quarter of a degree of the peak that came; the shorter ochre bar filled inside it is the same test with a tighter tolerance. Beside each bar is the issue where that happened and its value against what came; a row that never settled says so in words instead of drawing a bar.", "special": "It turns the whole archive into one answer to a practical question: how many months before the peak the number had, in the end, stopped moving. That is judged after the peak is known - at the time no one could tell a settled forecast from a pause - and the card for the event now running says exactly that.", "src": "All parsed IRI/CPC plume issues since 2002 against NOAA's official values; the settling test is our arithmetic."},
    "track/summer": {"title": "Past summers at this same point", "what": "How far the summer forecast landed from what came, in every past summer that made the same kind of call.", "see": "Buttons choose the month of the issue - June, July or August - and which summers are counted. One horizontal scale of the miss in degrees, with a heavy line at zero marked as what came: dots to its left were read low, dots to its right read high, each labelled with its issue, and the faint outlined ones are summers where a warm winter was called and no event followed. The dashed diamond is the middle case of the sample and is labelled on the chart as our arithmetic.", "special": "The sample shown by default is chosen by what the centres said, not by what happened, which is what lets it speak about a forecast standing today; the other button keeps only the summers an event followed and so cannot say how often a warm call came to nothing. With this few cases the middle value is a direction, not a correction to apply.", "src": "IRI/CPC plume issues since 2002 against NOAA's official values; the sample and the median are our arithmetic."},
    "track/today": {"title": "What the centres say today", "what": "Today's forecast for the peak of this event - the average over all the models of the newest issue - placed on one scale of degrees above normal.", "see": "One horizontal scale in degrees: the shaded band runs from the lowest model to the highest, the big ochre dot is the combined forecast for the peak season, and the red vertical line is the highest value the official record holds, with the season it belongs to named. The four cards below give that same peak, how wide the disagreement between models is, how many issues in a row it has been raised, and the distance above the record.", "special": "Every other tab here measures past errors, and all of those were measured below the record line drawn on this one - so this tab exists to say where today's call stands, not to suggest past misses can be applied to it. The band is how far the centres are from each other, not an error bar.", "src": "The newest IRI/CPC plume issue, monthly, with NOAA's official three-month values for the record line."},
    "trend/background": {"title": "The heat the ocean already holds", "what": "How much heat is stored in the ocean down to 700 and to 2000 metres, three months at a time from the start of each record — the condition the whole system was in before this event started.", "see": "Two stacked panels, the deeper layer above and the shallower below, each a single line from the start of its record to the latest three-month value, with a dot on the end and decades marked along the bottom. The labels beside each panel give the latest value, whether it is a record for that series, and the rise over the last ten years. The cards repeat that and add a figure for the planet's energy imbalance, which is quoted from a published paper rather than measured by us — it is flagged as quoted, and a warning note under the cards says so again.", "special": "Neither number is an El Niño measure. They are here to explain why statistical models trained on the past keep coming in low: the system they were fitted on held less heat than the one running now.", "src": "NOAA NCEI ocean heat content, three-month means against the 1955–2006 average; the energy imbalance figure is quoted literature, not a live series"},
    "trend/index": {"title": "Our risk index, update by update", "what": "The panel's own risk number from 0 to 100, plotted once for each day the panel has run, with a cut-down version of the same number worked out for past events.", "see": "The warm line is the index, one point per day the panel ran, on a scale of 0 to 100 down the left; a point is marked in a different colour on a day when the loudest kind of alert fired. The cool dashed line, on the same 0-100 scale, is the share of forecast models sitting below observed reality. Below the chart, one card per event compares today with the same date in past events on the cut-down core, and a second row does the same on the RONI scale, which subtracts the warm background.", "special": "The index is ours, not anyone's standard, so this is where it is held to account against events whose outcome is already known. Only rules that can be worked out for 1982 go into that core, so the core is always smaller than the full index shown elsewhere on the panel — core against core is a fair comparison, core against the full index is not.", "src": "The panel's own snapshots, one left behind by every update, plus the model count from the monthly forecast plume"},
    "trend/months": {"title": "The last thirteen months, ranked", "what": "A table of recent months giving each series' distance from normal for that month and where that month stands among all the years on record.", "see": "One row per month, oldest at the top and the current, still incomplete month at the bottom. Each series gets two columns: the month's distance from normal, and its rank out of the number of years the series covers. A month printed in red is one that became the warmest of its calendar month in the whole record.", "special": "The daily charts show shape and movement; this shows standing. It is the plainest statement the panel can make about level — not \"rising\" but \"warmest such month in the record\" — and it puts the three global series and the land boxes side by side on the same months.", "src": "The same daily series as the charts, averaged by calendar month; the land columns are ERA5 boxes, each an average of a small grid inside the box"},
    "trend/spectral": {"title": "A watch for short repeating rhythms", "what": "A table that tests every daily series on the panel for a regular rhythm of a few days repeating over the last 30 days.", "see": "One row per series — two dozen of them, from Niño 3.4 to individual moorings, each with the 30-day window it was tested on — starting with a verdict of none, weak, candidate or signal. Then one column for each period from 2 to 7 days holding the strength of that rhythm against ordinary noise: amber at the 95 per cent level, red at 99. The right-hand columns give the whole 2-7 day band as a share, how that compares with the series' own past windows, and the same calendar window in 1982, 1997, 2015 and 2023; the heading above says how many 99 per cent lines are showing now against how many chance alone would produce.", "special": "This is a hypothesis under observation, not a finding: the idea that a regular short rhythm shows up before an abrupt change. Its own scepticism is built into the table — roughly one window in fourteen carries a 99 per cent line by chance, so with two dozen series one or two are always lit, and a row is only called a signal if it is very strong, forms a comb of two periods, or holds on the same period three updates running.", "src": "Our own test over the panel's daily series, rebuilt by the daily run; the caption under the table gives the time it was last built"},
    "trend/sst_nino34": {"title": "The El Niño box, day by day", "what": "The daily temperature of the patch of central Pacific the world reads El Niño from, drawn as its distance from the normal for that day of the year.", "see": "A dark line runs 400 days back, the last 30 days picked out thicker and warmer, with a dot on the latest day. Behind it lie two shaded bands covering every year of the record on the same day of the year: the pale outer one is the full range those years ever reached, the darker inner one is where 10 to 90 per cent of them sat. To the right of today a vertical dashed line opens a year of space, where four coloured lines show what the events of 1982, 1997, 2015 and 2023 did from this same day of their own calendar, and a small warm wedge carries the 14-day outlook with its high, middle and low values.", "special": "This is the series that defines the event; everything else on the panel is context for it. Neither thing drawn to the right of the dashed line is a forecast by a forecasting centre: the coloured lines are what past events actually did, and the wedge is our own arithmetic — today's level plus the move every past year made over these same fourteen days of the calendar, at its low, middle and high.", "src": "Daily OISST sea-surface temperature via climatereanalyzer.org, read at every update"},
  ...
  /* СЦЕНЫ БЕЗ ПОЛЯ ГРАФИКА ТОЖЕ ОБЪЯСНЯЮТСЯ. Таблица товаров, шар со слоями, доска моделей —
     там нет `.plot`, и кнопке «i» некуда встать. Тогда она встаёт у названия сцены: место одно,
     смысл тот же. Проверка идёт после отрисовки, потому что до неё неизвестно, будет ли поле. */
  function plainAtTitle() {
    var head = document.querySelector('.stage-h'); if (!head) return;
    var old = head.querySelector('.plain-i'); if (old) old.remove();
    if (document.querySelector('.stage-body .plot .plain-i')) return;
    var h = plainBtn(S.view, S._subKey || '', 'title-i');
    if (h) head.insertAdjacentHTML('beforeend', ' ' + h);
  }
  function plainBtn(view, subk, cls, asSpan) {
  ...
  function render() {
    ...
    plainAtTitle();                          // «i» у заголовка там, где поля графика нет (15.09)
