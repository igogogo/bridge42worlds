 ...
 */
(function () {
    // ПЛАШКА ОТКЛЮЧЕНА 2026-08-19 по решению владельца: «версия 0.8 · предзапуск» больше
    // не соответствует действительности — сайт работает, статей шесть тысяч, разделы
    // наполнены. Обещание «запуск через две недели», данное 31 июля, тоже устарело.
    // Выключаем здесь, а не вырезаем <script> из шестнадцати шаблонов: правка одного
    // файла видна сразу, без пересборки ста шестидесяти тысяч страниц. Теги уберём
    // ближайшей общей пересборкой, вместе с остальными правками шаблонов.
    var B42_BETA_OFF = true;
    if (B42_BETA_OFF) return;
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        markLogo();
        // Карточка предзапуска БОЛЬШЕ НЕ ВЫСКАКИВАЕТ сама на первом заходе (владелец
        // 2026-08-02: «убери это окно при первом заходе, кто хочет — нажмёт»). Значок «!»
        // у логотипа остаётся всегда: кому интересно, в каком состоянии проект и как
        // прислать отзыв, — откроет сам. Всплывашка на входе закрывает ленту тому, кто
        // пришёл читать, и это худший первый экран из возможных.
