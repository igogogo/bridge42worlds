...
(function () {
    /* .topnav — своя шапка гида и авторских работ. Раньше эти страницы носили шторку,
       вбитую в шаблон: шесть английских слов на всех пяти языках и свой порядок
       (владелец 06.09). Теперь общая. */
    var bar = document.querySelector('.top-bar') || document.querySelector('.topnav');
    if (!bar) return;
    ...
    var NAV_I18N = {
        ...
        // читатель любого языка видел в столбце латинские «dashboard» и «formulas».
        archive:    { ru: 'Лента',             en: 'Archive',         es: 'Archivo',              ar: 'الأرشيف',       fr: 'Archives' , zh: '归档'},
        formulas:   { ru: 'Формулы',           en: 'Formulas',        es: 'Fórmulas',             ar: 'الصيغ',         fr: 'Formules' , zh: '公式'}
    };
    var MENU_I18N = { ru: 'Меню', en: 'Menu', es: 'Menú', ar: 'القائمة', fr: 'Menu', zh: '菜单' };

    function label(key) {
        // «Понятия» живут в словаре под старым ключом laws (раздел переименован 24.08,
        // ключ остался): и 'concepts', и 'laws' должны давать одну подпись, иначе в
        // столбце появляется латинское «concepts» среди переведённых (поймано 06.09).
        if (key === 'concepts') key = 'laws';
        var t = NAV_I18N[key];
    ...
    /* ПОРЯДОК ОДИН НА ВЕСЬ САЙТ. Владелец 06.09: «отсортируй пункты меню, чтобы было
       логично, и написание одинаковое: Home первая ссылка, потом About и так далее».
       Четыре смысловые группы подряд, без заголовков: сам сайт → знание → исследование →
       общее. Этот же список берёт js/search.js для страниц ленты, поэтому меню везде
       одинаковое; менять порядок нужно здесь и только здесь. */
    function navItems(L) {
      return [
        ...
        ['formulas', '/lang/' + L + '/formula/'],
        ['scientists', '/lang/' + L + '/scientists/'],
        ['sections', '/lang/' + L + '/sections/'],
        // Авторы собраны только по-английски (имена не переводятся, −600 МБ на выкладке):
        // ведём читателя любого языка в en, а не прячем раздел.
        ['authors', '/lang/en/authors/'],
        ['graph', '/lang/' + L + '/concepts/graph.html'],
        ['archive', '/lang/' + L + '/archive/'],
        ...
        // тем, кто ищет, но не спорит за внимание с лентой.
        ['community', '/lang/' + L + '/community/']
      ];
    }
    window.B42NavItems = navItems;
    ...
    // Своя шторка шаблона (.topnav-more) уступает место общей: удаляем её целиком.
    var own = bar.querySelector('.topnav-more');
    ...
    var items = navItems(L);
    // (прежний список пунктов убран: он жил здесь и ещё раз в js/search.js)
