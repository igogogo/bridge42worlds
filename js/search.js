...
// СПИСКИ ВСЕГДА «ПОПУЛЯРНО» (владелец 15.09.2026). Уровень ленты хранился в браузере
// (b42_version) и переписывался САМИМ ФАКТОМ открытия статьи в другом уровне — строка ниже
// по файлу делала это при загрузке страницы. Стоило один раз зайти в «Подробно», и все
// списки навсегда показывали технический текст: «Работа моделирует спектральные карты и их
// временную изменчивость…» вместо «Сердце у чёрной дыры бьётся неровно…». Переключателя в
// ленте нет с тех пор, как его убрали из меню, — ни увидеть это, ни отменить читатель не мог.
//
// Список — поверхность для выбора, а не для чтения: там нужен живой текст, по которому
// понятно, о чём работа. Глубину читатель выбирает уже в самой статье, её кнопки уровней
// работают как прежде. Пересчёта правка не требует: все три уровня уже лежат в базе,
// список просто просит нужный параметром.
var FEED_VERSION = 'popular';
function effVersion() { return FEED_VERSION; }
...
fetch('/config.json')
    ...
    .then(function(data) {
        /* Список языков — ОДИН на проект, из config.json. Раньше он был вписан
           руками в восьми местах клиента и воркера, и добавление шестого языка
           означало найти их все (владелец 30.08: «всё подготовь, чтобы не было
           хардкода»). Кладём в window сразу, как приехал: другим скриптам он
           нужен позже, и они спрашивают именно отсюда. */
        if (data && data.languages && data.languages.length) {
            window.B42_LANGS = data.languages;
        }
...
function cardHTML(item) {
    ...
    var url = base + (LVL_FILE[FEED_VERSION] || 'index.html');
    ...
    var _likeId = item.id + '_' + lang + '_' + FEED_VERSION;
...
/* Запасной список нужен: config приезжает не мгновенно, а совет может
   понадобиться раньше. Он же и документирует нынешний состав. */
var COUNCIL_LANGS = window.B42_LANGS || ['ru', 'en', 'es', 'ar', 'fr'];
var COUNCIL_TXT = {
...
function collapseNavOverflow() {
    ...
    // ПОНЯТИЯ — первым: главный раздел знания, а в шторке его не было вовсе
    // (владелец 28.08: «где в меню ссылка на понятия»). Формулы и граф вели в
    // СТАРЫЕ разделы: /formulas/ собран 4 августа и с тех пор не трогался, живой
