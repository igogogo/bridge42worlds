 ...
 *   • метку ссылки из НАШЕГО же адреса (?src=iri, utm_source) — по ней видно, какая
 *     розданная ссылка сработала; чужие параметры сюда не попадают;
 ...
 *
 * Файл подключается из js/search.js (собранные страницы) и js/sitenav.js (отдельные
 * страницы с нашей шапкой — панель El Niño, «Учиться», справочник), то есть появляется
 * без пересборки сайта. Двойной загрузки не боимся: проверка window.b42Metrics стоит
 * первой строкой, а скрипты выполняются по очереди.
 */
(function () {
    'use strict';
    ...
    // ОТКУДА ПРИШЛИ ПО ССЫЛКЕ. Домен реферера мы записываем и раньше, но он молчит там,
    // где он нужнее всего: ссылку прислали в письме, в мессенджере, в PDF — реферера нет
    // вовсе. Поэтому читаем метку из самого адреса: ?src=iri, ?from=letter, utm_source.
    // Владелец 10.09: «включить подробный мониторинг по их ссылкам».
    // Оставляем только буквы, цифры, точку, дефис и подчёркивание: метка попадёт в отчёт,
    // и произвольная строка из чужой ссылки там не нужна.
    function campaign() {
        try {
            var q = new URLSearchParams(location.search);
            var src = q.get('src') || q.get('from') || q.get('utm_source') || '';
            var camp = q.get('utm_campaign') || '';
            var s = String(src) + (camp ? ':' + camp : '');
    ...
    window.b42Metrics = {
        send: send, uid: uid,
        // ДЛЯ СТРАНИЦ, КОТОРЫЕ НЕ ПЕРЕЗАГРУЖАЮТСЯ. Панель El Niño — один адрес с вкладками:
        // читатель час ходит по разделам, а у нас записан один просмотр. Пусть страница
        // сама скажет, какой раздел открыли:  b42Metrics.screen('satellite').
        // Название — латиницей, коротко, устойчиво: оно попадёт в отчёт как есть.
        screen: function (name) { send('screen', String(name || '').slice(0, 40)); },
        // Нажатие, которое стоит считать: скачали файл, включили слой, открыли источник.
        action: function (name) { send('click', String(name || '').slice(0, 40)); },
