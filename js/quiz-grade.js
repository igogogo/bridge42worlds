...
(function (global) {
    ...
    function parseValue(raw) {
        ...
        s = fixSeparators(s);
        // Слова слева («порядка 10^33», «about 10^5») отбрасываем ДО разбора степени: иначе
        // последняя буква слова оказывается тем самым знаком, по которому мы решаем, что
        // мантисса опущена, и слово на латинскую «e» («more 10^5») ломало разбор молча.
        s = s.replace(/^[^\d+\-.]+/, '');
