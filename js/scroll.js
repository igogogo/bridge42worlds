...
async function initScroll() {
    ...
    var version = path.indexOf('advanced.html') !== -1 ? 'advanced'
                : (path.indexOf('simple.html') !== -1 ? 'simple' : 'popular');
    ...
    if (currentId) viewedIds.add(currentId);
...
function renderRelated(currentId, lang) {
    ...
    box.innerHTML = '<h3 class="related-h">' + (box.dataset.label || 'Related') + '</h3>' +
        scored.map(function(x){ return '<a href="' + x.a.url + '" class="related-item">' + x.a.title + '</a>'; }).join('');
}
...
function updateNextButton() {
    ...
    if (next) {
        btn.textContent = label + ': ' + next.title.substring(0, 60) + '... →';
