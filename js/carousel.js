...
(function () {
    ...
    function partsOf(img) {
        ...
        if (!m) return null;
        /* КАДРЫ ЛЕЖАТ ТОЛЬКО В РУССКОЙ ПАПКЕ. Это рисунки из самой работы — графики,
           схемы, микроскопия, — и языка у них нет; складывать пять одинаковых копий
           значило бы занять полгигабайта ради ничего. Но карточка на английской ленте
           зовёт /lang/en/…/c_0.webp, и там 404: карусель прицеплялась и застывала с
           пустым слоем (владелец 31.08: «карусель вырубилась на миникарточках»).
           Ведём все языки на русскую папку. */
        var base = src.slice(0, src.lastIndexOf('/') + 1)
                      .replace(/\/lang\/[a-z]{2}\//, '/lang/ru/');
        return { base: base, id: m[1] };
    }
