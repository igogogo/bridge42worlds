...
def license_label(lic_url):
    ...
    if "by-sa/4.0" in u:
        return "CC BY-SA 4.0"
    # BY-ND именуется наравне с остальными: класс у неё «не свободная» (производные
    # запрещены), но это не повод отдавать читателю безымянное «license» — подпись
    # говорит, ЧТО за лицензия, а что мы под ней показываем, решает класс.
    if "by-nd/4.0" in u:
        return "CC BY-ND 4.0"
    if "by/4.0" in u:
        return "CC BY 4.0"
