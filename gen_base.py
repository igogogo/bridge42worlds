...
def dir_for(lang):
    """HTML dir= атрибут для <html> — раньше вообще не выставлялся (только html[lang=ar]{direction:rtl}
    в CSS), из-за чего [dir="rtl"]-селекторы (нужны для RTL-зеркалирования слайдера версий и т.п.)
    не на что было ловить."""
...
# угадывать (юзер 2026-07-28). Строки узнаются мгновенно и выстраиваются в лестницу.
VERSION_ICONS = {
    "mini": _SVG.format('<path d="M9 12h6"/>'),
    "simple": _SVG.format('<path d="M5 12h14"/>'),
    "popular": _SVG.format('<path d="M5 9h14"/><path d="M5 15h9"/>'),
    "advanced": _SVG.format('<path d="M5 7h14"/><path d="M5 12h14"/><path d="M5 17h8"/>'),
}
...
def safe(s):
    if not s:
        return ""
    # Литеральный "обратный слэш + n" в тексте — старая болезнь генерации: модель
    # изредка пишет его ВНУТРИ строки JSON, и после разбора он остаётся двумя
    # символами. Читатель видел «harmony. \\nFaced» (владелец 26.08).
    # Данные не переписываем — чистим на выводе, в одной точке на весь генератор.
    #
    # НО НЕ ВНУТРИ ФОРМУЛ. Слепая замена съедала любую команду LaTeX, начинающуюся с n:
    # \nu превращалась в « u», \nabla в « abla», \neq в « eq» (владелец 10.09 на работе
    # OpenAI: «поправь формулу»). Все такие команды продолжаются СТРОЧНОЙ латиницей, а
    # случайный перенос от модели — пробелом, прописной буквой или знаком препинания.
    # По этому и различаем: чинит обе беды сразу и не требует списка команд.
    return _NL.sub(" ", str(s).replace("$", "$$"))
...
def level_switch_links(lang, current, date_str, aid, compact=False, with_mini=False):
    ...
    # чего, неудачный текст»).
    #
    # Он был не ступенью шкалы, а другим жанром — крючком для соцсети, — и стоял в одном
    # ряду с «просто» и «подробно». Человек нажимал «мини», ожидая самое простое, а получал
    # тизер, который без контекста читается тяжелее нормального начала. Плюс тот же текст
    # шапкой висел над «просто», и простое начиналось не с простого.
    #
    # Осталась честная лестница: просто → популярно → подробно, где ступени отличаются
    # глубиной, а не жанром. Сам текст мини из данных не удаляем — он уходит в карточки
    # ленты и в описание для соцсетей, где этот жанр к месту.
    order = LEVEL_ORDER
...
def mini_header_html(mini_text, lang):
    """Шапки «коротко · 20 секунд» больше нет — см. причину у order в level_switch_links.

    Функция оставлена пустой заглушкой, а не удалена: её зовёт генератор страницы, и
    выпиливать вызов ради трёх строк значит трогать то, что и так работает.
    """
    return ""
    if not mini_text:
        return ""
