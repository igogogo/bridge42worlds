...
ABBR_HEAD = re.compile(r"^[A-ZА-ЯЁ]{2,}")


def keeps_case(name):
    """Имя начинается с аббревиатуры — регистр первой буквы задан не нами, трогать нельзя.
    Аудит 16 августа, находка 3: «ДНК»→«дНК», «LIGO»→«lIGO», «МРТ простаты»→«мРТ простаты»,
    «ЯМР-спектроскопия»→«яМР-спектроскопия». Признак аббревиатуры — две заглавные подряд
    в начале: у обычного слова («Спектроскопия») вторая буква строчная."""
    return bool(ABBR_HEAD.match((name or "").strip()))
...
# СТРОЧНАЯ латинская, а не любая. Имя команды LaTeX целиком в нижнем регистре:
# \nu, \frac, \rho, \times, \beta. Заглавная сразу после — верный признак того,
# что это перенос строки и новое предложение: «\nIn everyday terms».
# Цена прежнего «любая латинская» — 105 тысяч литеральных \n в текстах на
# английском, испанском и французском (4 713 статей, замер 29.08). По-русски
# после переноса идёт кириллица, поэтому там их всего 141 — и ошибка пряталась
# ровно в тех языках, которые мы читаем реже. Проверка по корпусу: заглавная
# после \n встретилась 13 456 раз и НИ РАЗУ не была командой, строчная — 10 раз
# и каждый раз была (\nu в формулах).
...
def _fix_escapes(t):
    ...
    СТРОЧНАЯ латинская буква, у настоящего переноса — что угодно другое. Именно латинская:
    проверка на «любую букву» ломала «\\nстрока» — перенос строки перед кириллицей
    встречается в наших текстах постоянно, а LaTeX-команд кириллицей не бывает. И именно
    строчная: имя команды целиком в нижнем регистре, а «\\nIn everyday terms» — это перенос
    и новое предложение. Пока проверялась любая латинская, английский, испанский и
    французский тексты получали литеральное «\\n» посреди абзаца — 105 тысяч штук."""
    out = []
    i, n = 0, len(t)
    while i < n:
        c = t[i]
