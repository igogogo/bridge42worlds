...
def clean_json(t):
    ...
    t += "}" * (t.count("{") - t.count("}"))
    t += "]" * (t.count("[") - t.count("]"))
    # Одиночный слэш перед символом, который не является JSON-эскейпом («80\%», «\,», «\;»),
    # роняет json.loads целиком — ответ пропадает молча, а на повторе модель пишет то же самое.
    # Поймано 2026-08-05 на переводе аннотаций: так терялся 1 абстракт из 10.
    # Валидные эскейпы (\" \\ \/ \b \f \n \r \t \uXXXX) не трогаем, поэтому latex \\frac цел.
