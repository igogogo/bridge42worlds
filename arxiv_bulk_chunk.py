...
def main():
    ...
    # Исходник больше не нужен: работаем только с чанками, а дамп весит 5 ГБ и лежит мёртвым
    # грузом до следующего обновления. Владелец 2026-08-06: «будешь обновлять — качай,
    # разбирай, потом удаляй», и не в корзину, иначе место так и остаётся занятым.
    # К тому времени в Downloads успело накопиться ДВА одинаковых дампа на 10 ГБ — просто
    # потому, что второе скачивание получило имя с «(1)» и никто не заметил.
    # Удаляем только при полном успехе: replaced > 0 и разбор дошёл до конца без исключения.
    if replaced and not os.environ.get("B42_KEEP_DUMP"):
        try:
            size = SRC.stat().st_size
            SRC.unlink()
            print(f"🧹 исходный дамп удалён: {SRC.name} ({size / 2**30:.2f} ГБ освобождено)")
            print("   Нужен снова — качается с Kaggle; оставить его на диске: B42_KEEP_DUMP=1")
        except OSError as e:
