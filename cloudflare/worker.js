...
const WATCHERS = [
  // Сутки с запасом. Стоял час — по причине верной: 9 августа сторож два с половиной
  // часа крутился вхолостую после сетевого сбоя, и письмо автора пролежало непрочитанным.
  // Но 12.09 выяснилось, чем платим за частоту: сторож опрашивал пять ящиков раз в две
  // минуты и, когда доступы перестали подходить, продолжил с той же частотой — 2354
  // отвергнутых входа подряд выглядят подбором пароля, и хостинг закрыл почту домена.
  // Владелец 12.09: «сторож пусть работает раз в день». Порог тревоги идёт следом:
  // оставь его часом — она звенела бы всегда, и её перестали бы замечать.
  // Плата понятна и принята: ответ автора может пролежать до суток.
  { key: "mail", title: "Сторож почты", maxHours: 26 },
...
// «star» — ЗАЯВКА НА ПОЛНЫЙ РАЗБОР, а не оценка. Баннер на экспресс-статье обещает:
// «добавьте ★ в избранное, если хотите её ускорить». Обещание было пустым — избранное
// жило только в localStorage браузера, и никто о просьбе не узнавал (владелец 01.09:
// «вот я её кстати и добавил — у тебя есть механизм очереди?»). Теперь звезда на
...
// Порог сходства. Замер 08.09 на живых вопросах: панель отвечает уверенно (0.55-0.75),
...
3. Сначала то, что показывает панель (данные сегодня), потом то, что говорят работы.
   Если раздел ПАНЕЛЬ пуст, отвечай по работам — но пометки обязательны и там.
