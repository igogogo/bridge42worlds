...
FILES = (
    # Индекс статей теперь публикуется ОДИН — русский: его читает воркер (сторож публикации
    # и ежедневная сводка). Остальные языки и уровни чтения с 09.09 на сайт не едут вовсе,
    # и проверять их значило бы каждый раз краснеть на файлах, которых там нет по решению.
    ["lang/ru/articles-index.json"]
    + [f"lang/{l}/articles-latest.json" for l in LANGS]
