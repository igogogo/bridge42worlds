...
#
# cs.* ОТСЮДА УБРАН 12.09.2026. Ограничение стояло здесь как вторая, числовая копия
# правила из промпта отбора («из cs берём только ИИ как инструмент естественной науки»).
# Владелец 11.09 снял правило из промпта — «убери про ИИ, всё что важное берём», — а копия
# осталась и продолжала тихо делать своё: работа про сам ИИ далека от нашего физического
# корпуса, значит близость ниже медианы, значит до промпта она не доходила вовсе.
# Одно правило в двух местах и разошлось ровно так, как обычно расходится.
RESTRICTED = ("math.", "stat.", "econ.", "q-fin.")

# ОБЛАСТИ УПОРА (владелец 12.09): их НЕ режем по «чужому профилю». Отсев снизу считает
# чужим то, что далеко от корпуса, — а корпус у нас физический, и работы про старение и
# про ИИ далеки от него ПО ПОСТРОЕНИЮ. Пока фильтр учится на прошлом, упор на новое
# невозможен: он сам себя отменяет. Дубли темы (верхний срез) им по-прежнему отсекаются,
# и дальше их судят реранкер и модель — то есть послабление здесь не «берём всё подряд».
ACCENT = ("q-bio.", "cs.")
...
def prefilter(articles, keep_min=40):
    ...
    def accent(a):
        """Работа из области упора: нижний срез её не касается — см. ACCENT."""
        return str(a.get("primary_category") or "").startswith(ACCENT)

    kept = [a for a, s in zip(articles, near)
            if s <= p_dup and (s >= p_off or accent(a))]
...
def rerank_cut(articles, keep_frac=0.5, keep_min=30, model="8B"):
    """Второй этаж отбора: реранкер режет то, что вектор резать не умеет.

    РАЗНИЦА МЕХАНИЗМОВ, а не мощности. Вектор — би-энкодер: он превращает кандидата
    и наш корпус в числа ОТДЕЛЬНО и меряет расстояние. Поэтому он отвечает на вопрос
    «наше ли это по теме» и НЕ отвечает на вопрос «интересно ли это читателю»: у него
    нет места, где эти два текста встретились бы. Реранкер — кросс-энкодер, он читает
    описание и кандидата ВМЕСТЕ. Это ровно тот вопрос, который сейчас за большие деньги
    задаётся генеративной модели.

    ЗАМЕР (20 дней, 153 находки модели, rerank_eval.py). Если отдать модели верхнюю
    половину списка реранкера, у неё остаётся 82% её собственных прежних находок;
    случайная половина сохраняет 52%. Верхняя треть — 65% против 36%.

    Но recall — это совпадение, а не качество: модель всё равно вернёт своё число
    статей, просто других. Поэтому была слепая приёмка (rerank_blind.py): восемь дней,
    две подборки без пометок, оценка глазами до вскрытия ключа. **Семь дней из восьми
    за укороченный список.** Причина, кажется, в том, что реранкер вычищает нулевые
    работы (эррата, каталоги, узкие технические заметки), и модель тратит внимание
    на живых кандидатов, а не на отсев мусора.

    ОГОВОРКА, без которой числу верить нельзя: судья один, и это автор изменения.
    Слепота от предвзятости не защищает полностью — защищает от самой грубой её формы.
    Приёмку стоит повторить чужими глазами.

    БЕЗОПАСНОСТЬ — как у prefilter(): любой сбой возвращает список нетронутым.
    """
    if len(articles) <= keep_min:
        return articles, "кандидатов мало, реранкер не нужен"
    try:
        sys.path.insert(0, str(ROOT))
        import rerank_eval as rr
        key = rr.load_env()
        docs = [f"{a.get('title','')}. {str(a.get('summary',''))[:900]}" for a in articles]
        sc, stats = [], {}
        for i in range(0, len(docs), 16):
            sc += rr.rerank(rr.QUERY, docs[i:i + 16], key, model, stats=stats)
        from embeddings_build import log_usage
        log_usage("rerank", stats.get("tokens", 0), model=f"qwen3-reranker-{model.lower()}")
    except Exception as e:
        return articles, f"реранкер недоступен ({type(e).__name__}), беру всех"
    k = max(keep_min, int(len(articles) * keep_frac))
    order = sorted(range(len(articles)), key=lambda i: -sc[i])[:k]
    kept = [articles[i] for i in sorted(order)]
    return kept, f"реранкер оставил {len(kept)} из {len(articles)} (верхняя половина)"
