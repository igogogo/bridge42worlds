...
def av(path):
    """Адрес скрипта или стиля — БЕЗ метки версии.

    Метка тут была не зря: 27 августа страница графа грузила вчерашний
    b42-graph.js, и управление «не работало», хотя код лежал на месте. Но лечили
    не ту причину. Свежесть держат заголовки Worker: css и js отдаются с
    max-age=300 и ETag, то есть браузер перепроверяет их раз в пять минут и
    получает 304 в четырёх случаях из пяти. Версия в адресе поверх этого не
    добавляла ничего, зато делала страницу зависимой от байтов скрипта: правишь
    js — и все страницы понятий надо пересобирать, иначе они зовут старый адрес.
    31 августа я на это наступил дважды за час (владелец: «сделай, чтобы при
    правке css так не менялось»). Остальной сайт живёт без версий с 4 августа —
    теперь и этот раздел.

    Цена: окно рассинхрона до пяти минут. Значит новый скрипт обязан работать со
    старой разметкой, а новый стиль — не требовать класса, которого в выложенных
    страницах ещё нет.
    """
    return path
