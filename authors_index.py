...
def name_of(a):
    """`authors_parsed` — это [фамилия, имя, суффикс]. Берём ПОЛНОЕ имя, а не инициал.

    Первая версия склеивала «Фамилия И.» — казалось разумным: полное имя пишут
    по-разному, и без нормализации один человек рассыпается на трёх. На деле вышло
    наоборот и гораздо хуже: «Wang Y.» получил 4684 работы, то есть в одного человека
    слиплись сотни разных. Для указателя «кто работает у этой дырки» такая строка
    бесполезна — она ответит на любой вопрос и не значит ничего.

    Полное имя в данных есть у 84% авторов (замер на 21 тысяче подписей). Оставшимся
    16% достаётся инициал, и они по-прежнему склеиваются — но это шестая часть, а не
    основа указателя. Ошибка в сторону «слишком много разных людей» здесь безопаснее:
    лишний однофамилец виден глазом, а слипшиеся сотни — нет.
    """
