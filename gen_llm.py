...
def select_best(articles, date_str):
    ...
    # ПОТОЛОК КАНДИДАТОВ В ПРОМПТЕ — АБСОЛЮТНЫЙ, А НЕ ДОЛЕВОЙ. Оба среза выше режут
    # ДОЛЯМИ (перцентили дня, верхняя половина реранкера), и пока периметр был из
    # четырнадцати разделов, в промпт уходило ~375 кандидатов — терпимо. С 01.09 забор
    # снят, кандидаты идут из всех двадцати разделов arXiv: тот же долевой срез отдал бы
    # модели под тысячу работ, это больше сотни тысяч токенов в одном запросе. Отбор
    # упёрся бы в окно контекста и упал бы целиком — то есть день остался бы без статей.
    #
    # Список после реранкера отсортирован по его оценке, так что обрезаем хвост, а не
    # случайное. И ГОВОРИМ ВСЛУХ, сколько отрезали: молчаливое усечение читается как
    # «посмотрели всё», а посмотрели не всё.
    cap = int(CONFIG.get("select_max_candidates", 400))
    if cap and len(articles) > cap:
        print(f"  ✂️ кандидатов {len(articles)} — в промпт идут верхние {cap} по оценке "
              f"реранкера, {len(articles) - cap} отрезано (config.select_max_candidates)")
